...
- **i18n**: Custom i18n system — runtime in `src/lib/i18n.tsx`, translation data split into per-locale chunks in `src/lib/i18n-locale-chunks/` (45 files, one per locale). Only active locale + en/fr fallbacks loaded on startup; other locales fetched on demand when user switches language. Loader logic in `src/lib/i18n-data.ts` (~98 lines). Onboarding translations in `src/lib/i18n-onboarding.ts`. 45 locales supported. RTL handled for ar, he, ur, fa. SUPPORTED_LOCALES source-of-truth in `i18n-advanced.ts`. Super-app keys via `src/lib/i18n-data/translations-extra.ts` merged into GLOBAL_TRANSLATIONS. Onboarding extras via `src/lib/i18n-onboarding-extra.ts`. Islamic section via `i18n-islamic.ts` + `i18n-islamic-extra.ts`.
