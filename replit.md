...
- **Cache Layer**: `cache-layer.ts` — Two-tier cache: L1 in-memory LRU (500 entries) with per-domain TTL (profiles 5min, configs 1h, FX 15min, search 10min, media 30min) + L2 server_cache table via cache-manager edge function. `cachedFetch()` checks L1→L2→fetch→store both. `invalidateOnMutation()` clears L1 + server-side. Wired into FX rates (useCurrencyConversion).
- **Job Queue**: `job-queue.ts` — Client-side job queue with priority (critical/high/normal/low), concurrency control (3), exponential backoff retry (1s→30s cap), abort support. Server-side `job-runner` edge function (service-role auth) uses existing `job_queue` table (queue_name/scheduled_at/retry_count/max_retries columns) for email-batch, media-cleanup, analytics-aggregate, notification-dispatch, data-export, report-gen.
