...
- **i18n**: Custom i18n system — runtime in `src/lib/i18n.tsx` (~331 lines), translation data lazy-loaded from `src/lib/i18n-data.ts` (~6400+ lines, code-split). 45 locales supported (fr, en, es, de, it, pt, nl, pl, tr, ar, ja, ko, zh, hi, th, vi, id, ms, sv, da, nb, fi, el, cs, hu, ro, hr, bg, sk, he, uk, fa, bn, sw, tl, ur, am, ha, yo, wo, ru, sl, lt, lv, et). RTL handled for ar, he, ur, fa (CSS `:lang()` rules + `isRTL()` functions). SUPPORTED_LOCALES source-of-truth in `i18n-advanced.ts`. All 250 countries have multi-language supportedLanguages (zero en-only). Super-app keys (home.*, radar.*, orbit.nav.*, wallet.*, dashboard.*) translated for all 45 languages via `src/lib/i18n-data/translations-extra.ts` (35 languages) merged into GLOBAL_TRANSLATIONS. Onboarding (`ob.*`) keys translated for 38+ languages via `src/lib/i18n-onboarding-extra.ts`. Islamic section fully i18n'd via `i18n-islamic.ts` + `i18n-islamic-extra.ts` (12 tabs, all 45 languages). Merchant onboarding fully i18n'd with `mob.*` keys (90+ keys FR/EN).
...
- **Offline Storage Management** (Task #354): `quran-cache.ts` — user-configurable storage limit (10–500 MB, default 100 MB, persisted in localStorage). Offline manager shows total cache size in MB with progress bar against limit. Warnings at 80%+ (yellow) and 100%+ (red) of user-set limit, plus browser quota warnings. Storage limit enforced in single download, bulk download, and auto-download (favorites/bookmarks) paths. `bulkPinSurahs` also checks limit per-surah in the loop. Slider UI for adjusting limit in offline manager.
