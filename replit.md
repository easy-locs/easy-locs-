...
11. **Locale Codegen** (`scripts/generate-locales.ts`, npm script `generate:locales`) — Reads `src/lib/i18n-locale-chunks/*.json` filenames and generates `src/lib/i18n-locales.ts` (`APP_LOCALES` array + `AppLocale` type). `i18n-data.ts` uses `import.meta.glob` for dynamic chunk loaders. `i18n.tsx` re-exports `AppLocale` as `Locale`. Adding a new locale JSON file + running the script is all that's needed.
