 ...
 * Single responsibility: create, offer, accept, cancel, upload, visit, payment.
 */
...
export function useDealRoomMutations({
  ...
  const sendOffer = useMutation({
    mutationFn: async ({ amount, message, offerType, expiry }: {
      ...
      if (accepted && accepted > 0 && isOrgMember) {
        try {
          const { data: buyerProfile } = await supabase
            .from("profiles").select("email, name").eq("id", dealD.buyer_id).single();
  ...
  const cancelDeal = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("deal_rooms").update({ status: "cancelled" as any } as any).eq("id", deal!.id);
      if (error) throw error;
    },
