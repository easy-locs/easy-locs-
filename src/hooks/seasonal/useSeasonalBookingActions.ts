/**
 * useSeasonalBookingActions — Extracted booking request approve/reject/cancel/payment mutations.
 * Single responsibility: booking request action orchestration.
 */
...
interface UseSeasonalBookingActionsOptions {
  properties: { id: string; label: string; country?: string }[];
  reload: () => Promise<void>;
}

export function useSeasonalBookingActions({ properties, reload }: UseSeasonalBookingActionsOptions) {
  ...
  const approveRequest = useCallback(async (req: any) => {
    ...
    if (orgId && user) {
      await supabase.from("seasonal_bookings").insert({
        ...
        guest_name: req.guest_name, guest_email: req.guest_email,
  ...
  const sendPaymentLink = useCallback(async (req: any) => {
