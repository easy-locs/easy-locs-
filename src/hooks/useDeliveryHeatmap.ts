...
const GRID_SIZE_KM = 1.5;
const LAT_STEP = GRID_SIZE_KM / 111;
const LNG_STEP_AT_48 = GRID_SIZE_KM / (111 * Math.cos(48.85 * Math.PI / 180));
...
export function useDeliveryHeatmap(orgId?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (orgId) query = query.eq("merchant_id", orgId);

      const { data: jobs, error } = await query.limit(500);
      if (error || !jobs) { setZones([]); return; }
      ...
      for (const j of jobs) {
        ...
        const fee = j.current_price ?? j.quoted_price;
        const existing = clusters.get(key) || { lat: j.pickup_lat, lng: j.pickup_lng, count: 0, fees: [], recentCount: 0, currency: j.currency || "AED" };
        existing.count++;
        if (fee) existing.fees.push(fee);
      ...
      const pendingCount = jobs.filter(j => j.status === "searching").length;
      const hotCount = demandZones.filter(z => z.intensity >= 0.6).length;
      const allFees = jobs.filter(j => j.current_price || j.quoted_price).map(j => (j.current_price ?? j.quoted_price)!);
      const avgFee = allFees.length ? Math.round(allFees.reduce((a, b) => a + b, 0) / allFees.length * 100) / 100 : 0;
      const hourCounts: Record<number, number> = {};
      for (const j of jobs) { const h = new Date(j.created_at).getHours(); hourCounts[h] = (hourCounts[h] || 0) + 1; }
