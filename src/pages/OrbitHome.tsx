 ...
 * Structure: Search → Conversations → Quick Actions → Categories → Wallet → Activity
 */
...
import {
  MessageCircle, Phone, ChevronRight,
  Store, Package, Truck, Building2, Car, Plane,
} from "lucide-react";
...
/* ══════ Conversation Row ══════ */
const ConversationRow = memo(function ConversationRow({ name, message, time, unread, avatar, onClick }: {
  name: string; message: string; time: string; unread?: number; avatar?: string; onClick: () => void;
}) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all active:scale-[0.98] min-h-[56px]"
    ...
    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-1 px-1">
      {CATEGORIES.map((cat) => (
        <button
          ...
          className="flex flex-col items-center gap-1.5 min-w-[58px] py-2.5 px-1.5 rounded-xl active:scale-95 transition-transform"
          style={{ background: "hsl(var(--hud-surface))" }}
        >
          <span className="text-lg leading-none">{cat.icon}</span>
          <span className="text-[10px] font-semibold truncate w-full text-center" style={{ color: "hsl(var(--hud-text-dim) / 0.65)" }}>
            {cat.label}
          </span>
        </button>
      ))}
...
export default function OrbitHome() {
  ...
  return (
    <div className="flex flex-col items-center px-4 pt-3 pb-10 gap-4 min-h-full">
      ...
      {/* 2. Recent Conversations — central Orbit block */}
      <div className="w-full max-w-md">
        <SectionLabel title={t("orbit.home.section_priorities") || "RECENT"} action={t("orbit.card.contacts") || "All"} onAction={() => navigate("/orbit/contacts")} />
      ...
      <div className="w-full max-w-md">
        <SectionLabel title={t("orbit.home.section_shortcuts") || "SHORTCUTS"} />
        <CategoryRow />
      </div>

      {/* 5. Wallet Preview */}
      <div className="w-full max-w-md">
        <SectionLabel title={t("orbit.home.section_wallet") || "WALLET"} />
        <OrbitWalletCard />
      </div>

      {/* 6. System Status — minimal */}
