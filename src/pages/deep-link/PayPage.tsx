 ...
 * No login required to view. Login required to pay.
 * Supports: direct payment requests, QR-generated links, invoice links.
 */
...
export default function PayPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Try to find a payment request or wallet transaction link
      const { data: txn } = await (supabase as any)
        .from("wallet_transactions")
        .select("id, amount, currency, reference_type, reference_code, status, created_at, sender_id, recipient_id")
        .eq("id", paymentId)
        .maybeSingle();
      return txn;
    },
        ...
        <div className="max-w-md mx-auto px-4 pt-8 pb-24 space-y-6">
          {notFound ? (
            ...
            <>
              ...
              {/* CTA */}
              {user ? (
                <Link to={`/dashboard/wallet?action=send&amount=${Math.abs(amount || 0)}&currency=${currency}&ref=${paymentId}`}>
