...
import { Shield, User, Home, Briefcase, Building2, ArrowRight, ArrowLeft, MapPin } from "lucide-react";
          ...
          {[0, 1].map((s) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full transition-colors ${
                s <= step ? "bg-gradient-gold" : "bg-border"
              }`}
            />
          ))}
        </div>
                ...
                {profiles.map((p) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                      profile === p.type ? "bg-gradient-gold" : "bg-muted"
                    }`}>
            ...
            >
              ...
              <div className="space-y-3">
                {countries.map((c) => (
                  <button
                    key={c.code}
                    onClick={() => c.available && setCountry(c.code)}
