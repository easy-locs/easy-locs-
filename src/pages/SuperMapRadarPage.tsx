...
type SearchResult =
  | { kind: "shop"; id: string; title: string; subtitle: string; lat: number | null; lng: number | null; slug: string | null }
  | { kind: "product"; id: string; title: string; subtitle: string; lat: number | null; lng: number | null; shopSlug: string | null; productId: string };

type UserPos = { lng: number; lat: number; accuracy?: number };
  ...
  return {
    ...
    features: shops
      ...
      .map((s) => ({
        ...
        properties: { id: s.id, name: s.name || "Shop", slug: s.slug || "", city: s.city || "", description: s.description || "" },
      })),
  };
}
...
function circlePolygon(center: [number, number], radiusKm: number, points = 64) {
  ...
  for (let i = 0; i <= points; i++) {
    ...
    const lng2 = lng1 + Math.atan2(Math.sin(bearing) * Math.sin(dByR) * Math.cos(lat1), Math.cos(dByR) - Math.sin(lat1) * Math.sin(lat2));
...
function useRealtimeShops() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await (supabase as any)
      ...
      .eq("published", true);
    if (!error) setShops((data || []) as ShopRow[]);
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "storefront_pages" }, () => load())
      .subscribe();
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const [shopsRes, productsRes] = await Promise.all([
        (supabase as any).from("storefront_pages").select("id, name, slug, description, city, lat, lng").eq("published", true).ilike("name", `%${q}%`).limit(5),
        (supabase as any).from("catalog_items").select("id, title, description, price, currency, photo_url, shop_id, storefront_pages(id, slug, name, lat, lng)").eq("available", true).ilike("title", `%${q}%`).limit(5),
      ]);

      if (!active) return;

      const results: SearchResult[] = [];
      (shopsRes.data || []).forEach((s: any) => results.push({ kind: "shop", id: s.id, title: s.name || "Shop", subtitle: s.city || "Nearby", lat: s.lat, lng: s.lng, slug: s.slug }));
      (productsRes.data || []).forEach((p: any) => {
        const shop = p.storefront_pages;
        results.push({ kind: "product", id: `prod-${p.id}`, title: p.title || "Product", subtitle: fmtMoney(p.price, p.currency), lat: shop?.lat ?? null, lng: shop?.lng ?? null, shopSlug: shop?.slug ?? null, productId: p.id });
      });
      setItems(results);
      setLoading(false);
    }
...
export default function SuperMapRadarPage() {
  ...
  const nearbyShops = useMemo(() => {
    ...
    return shops
      ...
      .map((s) => ({ ...s, distanceKm: haversineKm(userPos.lat, userPos.lng, s.lat as number, s.lng as number) }))
      .filter((s) => s.distanceKm <= radarKm)
      .sort((a, b) => a.distanceKm - b.distanceKm);
  }, [shops, userPos, radarKm]);

  /* ── 1. Base map init (stable, no deps on shops/radarKm) ── */
  useEffect(() => {
    ...
    map.on("load", () => {
      /* 2. Radar ring */
      map.addSource(RINGS_SOURCE_ID, { type: "geojson", data: circlePolygon(DEFAULT_CENTER, 5) });
      map.addLayer({ id: "radar-fill", type: "fill", source: RINGS_SOURCE_ID, paint: { "fill-color": "#0ea5e9", "fill-opacity": 0.08 } });
      map.addLayer({ id: "radar-outline", type: "line", source: RINGS_SOURCE_ID, paint: { "line-color": "#38bdf8", "line-width": 2, "line-opacity": 0.85 } });

      /* 3. Shops source + pins */
      map.addSource(SOURCE_ID, { type: "geojson", data: makeShopGeoJson([]) });
      map.addLayer({ id: "shops-circle", type: "circle", source: SOURCE_ID, paint: { "circle-radius": 8, "circle-color": "#38bdf8", "circle-stroke-width": 2, "circle-stroke-color": "#ffffff" } });
      map.addLayer({ id: "shops-label", type: "symbol", source: SOURCE_ID, layout: { "text-field": ["get", "name"], "text-size": 11, "text-offset": [0, 1.3], "text-anchor": "top" }, paint: { "text-color": "#ffffff", "text-halo-color": "#031633", "text-halo-width": 1 } });
    ...
    map.on("error", (e) => console.error("[super-map] map error", e));
  ...
  const startLiveGeo = useCallback(() => {
    if (!navigator.geolocation) { setGeoError("Geolocation not available on this device."); return; }
    setGeoError("");
    if (watchIdRef.current != null) { navigator.geolocation.clearWatch(watchIdRef.current); watchIdRef.current = null; }

    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        const next: UserPos = { lng: pos.coords.longitude, lat: pos.coords.latitude, accuracy: pos.coords.accuracy };
        setUserPos(next);
        if (mapRef.current) {
  ...
  const handleResultClick = useCallback(
    (item: SearchResult) => {
      ...
      if (item.kind === "shop" && item.slug) {
        const shop = shopsRef.current.find((s) => s.id === item.id);
        if (shop) setSelectedShop(shop);
      }
    ...
    <div className="relative w-full h-screen overflow-hidden bg-background">
      ...
      <div className="absolute top-4 left-4 right-4 z-10">
        <div className="flex items-center gap-2 rounded-xl bg-card/90 backdrop-blur-md border border-border px-3 py-2 shadow-lg">
          ...
          />
          {query ? (
            <button onClick={() => setQuery("")} className="bg-transparent border-0 text-foreground p-0">
              <X className="h-4 w-4" />
            </button>
          ) : null}
        </div>
        ...
        <div className="flex items-center gap-2 mt-2 overflow-x-auto no-scrollbar">
          <button onClick={startLiveGeo} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-primary/20 text-primary border border-primary/30 backdrop-blur-sm shrink-0">
            <Navigation className="h-3 w-3" />
            Live GPS
          </button>
          <button onClick={centerOnUser} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-card/80 text-foreground border border-border backdrop-blur-sm shrink-0">
          ...
          {[1, 3, 5, 10].map((km) => (
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium backdrop-blur-sm shrink-0 border ${radarKm === km ? "bg-primary text-primary-foreground border-primary" : "bg-card/80 text-foreground border-border"}`}
            >
      ...
      {selectedShop && (
        <div className="absolute bottom-28 left-4 right-4 z-10 bg-card/95 backdrop-blur-md border border-border rounded-2xl p-4 shadow-xl">
          <div className="flex items-start justify-between mb-3">
            ...
            <button onClick={() => setSelectedShop(null)} className="bg-transparent border-0 text-muted-foreground p-1">
              <X className="h-4 w-4" />
            </button>
          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 bg-card/95 backdrop-blur-md border-t border-border">
        <div className="px-4 pt-3 pb-4 safe-area-pb">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            {geoError ? geoError : userPos ? "Realtime nearby discovery active" : "Tap Live GPS to start nearby radar"}
          </p>
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (mapRef.current && shop.lng != null && shop.lat != null) {
                    mapRef.current.flyTo({ center: [shop.lng, shop.lat], zoom: 15, speed: 0.9, essential: true });
                  }
                }}
              >
                <UniversalEntityCard
