...
export default function AdminMerchantHealthPage() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: merchants }, { data: products }, { data: promos }] = await Promise.all([
        (supabase as any).from("seed_merchants").select("*").limit(500),
        (supabase as any).from("seed_products").select("*").limit(5000),
        (supabase as any).from("seed_merchant_promos").select("*").limit(2000),
      ]);
