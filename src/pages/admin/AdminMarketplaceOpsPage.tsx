...
export default function AdminMarketplaceOpsPage() {
  ...
  const { data: merchantCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await (supabase as any).from("seed_merchants").select("*", { count: "exact", head: true }).eq("is_active", true);
      return count || 0;
    },
  });
  ...
  const { data: productCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await (supabase as any).from("seed_products").select("*", { count: "exact", head: true }).eq("is_available", true);
      return count || 0;
    },
  });
  ...
  const { data: featuredCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await (supabase as any).from("seed_merchants").select("*", { count: "exact", head: true }).eq("is_featured", true);
      return count || 0;
    },
  });
  ...
  const { data: categoryCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase as any).from("seed_merchants").select("category").eq("is_active", true);
