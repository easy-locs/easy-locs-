 ...
 * Now reads from the canonical `notifications` table.
 */
      ...
      <div className="flex items-center gap-3 px-4 pt-6 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Live notification queue from canonical notifications table
          </p>
        </div>
      </div>
            ...
            <div key={row.id} className="rounded-2xl border border-border/20 bg-card p-4 space-y-2">
              <div className="flex items-start justify-between">
                ...
                <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                  ...
                  {row.resolved ? "resolved" : row.read_at ? "read" : "unread"}
                </span>
              </div>
