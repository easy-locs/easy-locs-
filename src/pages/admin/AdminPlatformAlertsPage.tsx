...
import { fetchPlatformAlertsData } from "@/repositories/admin-ops.repository";
...
export default function AdminPlatformAlertsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const raw = await fetchPlatformAlertsData();
      return {
        openTickets: raw.tickets.filter((r: any) => r.status === "open").length,
        failedNotifications: raw.notifications.filter((r: any) => !r.read_at).length,
        disputedOrders: raw.orders.filter((r: any) => r.status === "disputed").length,
        unpaidOrders: raw.orders.filter((r: any) => ["unpaid", "pending"].includes(String(r.payment_status ?? ""))).length,
      };
    },
    staleTime: 10000,
  });
  ...
  return (
    <div className="min-h-[100dvh] bg-background pb-24">
      ...
      {isLoading && [1, 2].map((i) => <div key={i} className="mx-4 mb-3 h-16 rounded-2xl bg-muted animate-pulse" />)}
      {!isLoading && data && (
...
function AlertMetric({ title, value, danger }: { title: string; value: string; danger?: boolean }) {
  return (
