/**
 * RestaurantPage — Dedicated restaurant detail + grouped menu (no prices).
...
export default function RestaurantPage() {
  ...
  const { data: menuItems = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await (supabase as any)
        .from("menu_items")
        .select("id, name, description, image_url, category_name, menu_category, is_available, sort_order")
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col" style={{ background: "hsl(var(--background))" }}>
      ...
      {/* Menu sections */}
      {!isLoading && (
        <div className="flex-1 px-4 mt-5 pb-24 space-y-6">
