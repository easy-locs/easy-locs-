...
const ClientDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      setStats({
        bookings: (seasonalCount || 0) + (conciergeCount || 0) + (marketplaceCount || 0),
        messages: 0,
        documents: 0,
      });
      setLoaded(true);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          {quickCards.map((card, i) => (
            <motion.div key={card.path} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}>
              <Link
                to={card.path}
                className="group flex flex-col h-full bg-card rounded-xl p-5 shadow-card border border-border/50 hover:shadow-card-hover hover:border-accent/40 transition-all"
              >
