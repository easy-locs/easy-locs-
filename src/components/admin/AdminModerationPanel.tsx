...
import {
  fetchRiderPresence, fetchProfilesByIds, updateRiderPresenceStatus,
  insertAppNotification,
} from "@/repositories/admin.repository";
import { fetchDisputes, updateDispute, insertAuditLog } from "@/repositories/delivery.repository";
...
export default function AdminModerationPanel({ orgId }: { orgId: string }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [driverList, disputeList] = await Promise.all([
      fetchRiderPresence(),
      fetchDisputes(),
    ]);

    const typedDrivers = driverList as ModerationDriver[];
    const userIds = typedDrivers.map(d => d.user_id);
    if (userIds.length > 0) {
      ...
      if (profiles) {
        ...
        typedDrivers.forEach(d => { d.name = nameMap.get(d.user_id) || undefined; });
      }
    }

    setDrivers(typedDrivers);
    setDisputes(disputeList as EscalatedDispute[]);
    setLoading(false);
  }, [orgId]);
  ...
  const handleDriverAction = async (driverId: string, userId: string, action: ModerationAction) => {
    try {
      if (action === "suspend" || action === "ban") {
        await updateRiderPresenceStatus(userId, { is_online: false, is_available: false });
      }
                      ...
                      <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                        <div className="px-3 pb-3 space-y-2 border-t" style={{ borderColor: "hsl(var(--hud-border) / 0.06)" }}>
                          <Textarea value={actionNote} onChange={e => setActionNote(e.target.value)}
                            placeholder="Note de modération…" className="min-h-[50px] text-[10px] mt-2"
            ...
            disputes.map(dispute => {
              ...
              return (
                <div key={dispute.id} className="rounded-xl overflow-hidden"
                  style={{ background: "hsl(var(--hud-surface))", border: `1px solid ${isOpen ? "hsl(var(--warning) / 0.15)" : "hsl(var(--hud-border) / 0.06)"}` }}>
                  <div className="flex items-center gap-3 px-3 py-2.5 cursor-pointer"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[8px]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Par: {dispute.raised_by_role}</span>
                    ...
                    <span className="text-[8px] px-1.5 py-0.5 rounded-full font-semibold"
                      style={{ background: isOpen ? "hsl(var(--warning) / 0.1)" : "hsl(var(--success) / 0.1)", color: isOpen ? "hsl(var(--warning))" : "hsl(var(--success))" }}>
                  ...
                  <AnimatePresence>
                    {isSelected && (
                      <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                        <div className="px-3 pb-3 space-y-2 border-t" style={{ borderColor: "hsl(var(--hud-border) / 0.06)" }}>
                          {dispute.description && <p className="text-[9px] pt-2" style={{ color: "hsl(var(--hud-text-dim))" }}>{dispute.description}</p>}
                          {dispute.resolution && (
                          ...
                          {isOpen && (
                            <>
                              <Textarea value={resolutionNote} onChange={e => setResolutionNote(e.target.value)}
                                placeholder="Résolution du litige…" className="min-h-[50px] text-[10px]"
                                style={{ background: "hsl(var(--hud-bg))", borderColor: "hsl(var(--hud-border) / 0.15)", color: "hsl(var(--hud-text))" }} />
