...
import { Globe, MapPin, Briefcase, Home, Building, Palmtree, Users } from "lucide-react";
...
/** Group countries by continent for cleaner layout */
    ...
    <section className="border-t border-border bg-muted/20 py-12">
      <div className="max-w-[1400px] mx-auto px-4 space-y-10">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {CONTINENT_ORDER.map(cont => {
              ...
              return (
                <div key={cont.label}>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">{cont.label}</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {countries.map(c => (
                      <Link key={c.slug} to={`/country/${c.slug}`} className="text-xs px-2.5 py-1.5 rounded-lg bg-card border border-border hover:border-accent/40 hover:bg-accent/5 transition-colors text-muted-foreground hover:text-foreground">
                        {c.flag} {c.name}
                      </Link>
                    ))}
                  </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {cities.map(city => (
              <Link key={city.slug} to={`/city/${city.slug}`} className="text-xs px-2.5 py-1.5 rounded-lg bg-card border border-border hover:border-accent/40 hover:bg-accent/5 transition-colors text-muted-foreground hover:text-foreground">
                {city.name}
              </Link>
            ))}
          </div>
        ...
        {/* ── Categories (marketplace hierarchy) ── */}
        <div>
          <h2 className="text-lg font-bold text-foreground mb-5 flex items-center gap-2">
            ...
            {CATEGORY_HIERARCHY.map(group => (
              <div key={group.value}>
                ...
                <div className="flex flex-col gap-1">
                  {group.subcategories.slice(0, 6).map(sub => (
                    <Link
                      key={sub.value}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {SEO_SERVICE_CATEGORIES.map(svc => (
              <Link key={svc.slug} to={`/services/${svc.slug}`} className="text-xs px-2.5 py-1.5 rounded-lg bg-card border border-border hover:border-accent/40 hover:bg-accent/5 transition-colors text-muted-foreground hover:text-foreground">
                {svc.icon} {svc.label}
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { to: "/about", label: "About Easy-Locs" },
            ].map(link => (
              <Link key={link.to} to={link.to} className="text-xs px-3 py-1.5 rounded-lg bg-card border border-border hover:border-accent/40 hover:bg-accent/5 transition-colors text-muted-foreground hover:text-foreground">
                {link.label}
              </Link>
            ))}
          </div>
