      ...
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <button
            ...
            className="relative flex flex-col items-center justify-center pt-1.5 pb-1 min-w-0 flex-1 min-h-[44px] active:scale-90 transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-lg"
          >
            {badge > 0 && (
              <span
                className="absolute top-0 right-2 min-w-[16px] h-4 rounded-full text-[9px] font-bold flex items-center justify-center px-1 z-10"
                style={{ background: "hsl(var(--hud-danger))", color: "#fff" }}
              >
                {badge > 99 ? "99+" : badge}
              </span>
            )}
            {/* Uniform icon container — same size for ALL tabs */}
            <div
              className="w-9 h-9 rounded-full flex items-center justify-center"
              style={{
                background: isOrbit
                  ? isActive ? "hsl(var(--hud-cyan))" : "hsl(var(--hud-cyan) / 0.15)"
                  : "transparent",
              }}
            >
              <item.icon
