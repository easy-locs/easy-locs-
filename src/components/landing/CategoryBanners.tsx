 ...
 * live counters, depth effects, and smooth animations.
 */
...
const CATEGORIES = [
  ...
  {
    title: "Real Estate",
    sub: "Buy, sell, rent, manage — 190+ countries",
    icon: Building2,
  ...
  {
    title: "Services",
    sub: "Plumber, electrician, cleaning — nearby pros",
    icon: Wrench,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
            Discover, order, book, and manage — all from a single platform.
          </p>
        </motion.div>

        {/* Featured banner (first item - large) */}
        <motion.div
            ...
            >
              ...
              >
                <div className="aspect-[4/5] sm:aspect-[3/4] relative overflow-hidden">
                  ...
                  {cat.badge && (
                    <span className="absolute top-2 right-2 px-2 py-0.5 rounded-full text-[8px] font-bold text-white backdrop-blur-md"
                      style={{ background: `${cat.accent}70`, border: `1px solid ${cat.accent}30` }}>
                      {cat.badge}
                    </span>
                  )}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center backdrop-blur-md border border-white/10"
                        style={{ background: `${cat.accent}25` }}>
                        <cat.icon className="h-3.5 w-3.5" style={{ color: cat.accent }} />
                      </div>
                      <div className="min-w-0">
                        <h3 className="text-xs sm:text-sm font-bold text-white leading-tight truncate">{cat.title}</h3>
                        <p className="text-[8px] sm:text-[9px] text-white/50 truncate">{cat.count} {cat.countLabel}</p>
                      </div>
                    </div>
                    <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-[9px] sm:text-[10px] font-bold text-white backdrop-blur-md border border-white/10 group-hover:gap-1.5 transition-all"
