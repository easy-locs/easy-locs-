...
type JobType = "taxi" | "food_delivery" | "parcel_delivery";
type BookingMode = "now" | "scheduled";

const JOB_TYPES: { value: JobType; serviceLevel: string; label: string; icon: React.ReactNode }[] = [
  { value: "taxi", serviceLevel: "taxi_standard", label: "Taxi", icon: <Car className="h-5 w-5" /> },
  { value: "food_delivery", serviceLevel: "bike_delivery", label: "Delivery", icon: <Bike className="h-5 w-5" /> },
...
export function RideBookingForm() {
  ...
  const [jobType, setJobType] = useState<JobType>("taxi");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await createJob({
        jobType,
        serviceLevel: selectedType.serviceLevel,
        pickupLabel,
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="grid grid-cols-3 gap-2">
        {JOB_TYPES.map(rt => (
          <button key={rt.value} type="button" onClick={() => setJobType(rt.value)}
            className={cn("flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 transition-all",
              jobType === rt.value ? "border-primary bg-primary/5 text-primary shadow-sm" : "border-border/40 bg-card text-muted-foreground hover:border-border"
            )}>
            {rt.icon}
