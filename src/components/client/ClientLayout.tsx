  ...
  return (
    <div className="min-h-screen flex bg-background mobile-safe">
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <main className="app-main flex-1 p-3 sm:p-6 pb-20 lg:pb-6">{children}</main>
        <footer className="py-3 text-center border-t border-border/30 hidden lg:block">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden border-t border-border bg-card/98 backdrop-blur-lg" style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
          <div className="flex items-stretch justify-around h-14">
            {[
              { icon: LayoutDashboard, label: t("nav.dashboard") || "Dashboard", path: "/client" },
              { icon: Search, label: t("nav.explore") || "Explore", path: "/explore" },
              { icon: CalendarCheck, label: t("nav.bookings") || "Bookings", path: "/client/bookings" },
              { icon: MessageCircle, label: t("nav.messages") || "Messages", path: "/client/messages" },
