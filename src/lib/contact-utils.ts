...
/** Email mailto with prefilled subject and body */
export function emailLink(email: string, ctx: ListingContext): string {
  const subject = `Inquiry about ${ctx.title}`;
  const body = [
    "Hello,",
    "",
    `I am interested in this listing: ${ctx.title}`,
    ctx.price ? `Price: ${ctx.price}` : "",
    ctx.city ? `Location: ${ctx.city}${ctx.country ? `, ${ctx.country}` : ""}` : "",
    "",
    `Link: ${getListingUrl(ctx)}`,
    "",
