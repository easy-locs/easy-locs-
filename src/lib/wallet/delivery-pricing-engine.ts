...
function computeAIAdjustment(rawFee: number, signals: AISignals): { factor: number; reason: string } {
  ...
  if (ratio > 2.0) { adjustment += 0.15; reasons.push("high_demand"); }
  else if (ratio > 1.5) { adjustment += 0.08; reasons.push("moderate_demand"); }
  else if (ratio < 0.5) { adjustment -= 0.08; reasons.push("low_demand_discount"); }

  if (signals.avgEtaMinutes && signals.avgEtaMinutes > 30) { adjustment += 0.05; reasons.push("long_eta"); }
  if (signals.acceptanceRate != null && signals.acceptanceRate < 0.4) { adjustment += 0.10; reasons.push("low_acceptance"); }
  if (signals.conversionRate != null && signals.conversionRate > 0.7 && adjustment > 0.05) { adjustment = Math.min(adjustment, 0.05); reasons.push("conversion_protected"); }
  if (adjustment < 0) { adjustment = Math.max(adjustment, -0.10); reasons.push("margin_floor"); }

  const capped = Math.max(-AI_MAX_ADJUSTMENT, Math.min(AI_MAX_ADJUSTMENT, adjustment));

  return { factor: 1 + capped, reason: reasons.join(",") || "balanced" };
}
