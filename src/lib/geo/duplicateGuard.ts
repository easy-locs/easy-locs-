...
export async function checkStorefrontDuplicate(..._args: any[]): Promise<{ isDuplicate: false; blocked: false; existingMatch: null; result: { reasons: string[] } }> {
  return { isDuplicate: false, blocked: false, existingMatch: null, result: { reasons: [] } };
}

export async function checkServiceDuplicate(..._args: any[]): Promise<{ isDuplicate: false; blocked: false; existingMatch: null }> {
  return { isDuplicate: false, blocked: false, existingMatch: null };
}
