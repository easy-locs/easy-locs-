...
export async function computeShopHealth(shopId: string): Promise<ShopHealthScore> {
  const [shopRes, productsRes] = await Promise.all([
    (supabase as any).from("storefront_pages").select("*").eq("id", shopId).maybeSingle(),
    (supabase as any).from("products").select("id, name, price, image_url, description").eq("shop_id", shopId),
  ]);
