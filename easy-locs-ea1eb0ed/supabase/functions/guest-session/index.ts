...
Deno.serve(async (req) => {
  ...
  const rlResult = await checkServerRateLimit(req, "guest-session", { maxRequests: 20, windowSeconds: 60 });
        ...
        if (session.context_type === "service" && session.context_id) {
          ...
          if (!svc) {
            const { data: mSvc } = await supabase
              .from("listings").select("title").eq("id", session.context_id).maybeSingle();
