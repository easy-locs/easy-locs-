...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({ error: "Invalid credential payload — missing required fields" }),
    ...
    if (
      ...
      console.warn(
        `[WebAuthn] Credential cloning detected for user ${auth.userId}. ` +
          `Stored: ${storedCred.sign_count}, received: ${authDataParsed.signCount}`
      );
      return new Response(
    ...
    console.error("webauthn-authentication-verify error:", message);
