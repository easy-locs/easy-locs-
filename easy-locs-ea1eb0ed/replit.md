...
- **Automated evals** (`supabase/functions/ai-eval-runner`): service-role runner iterates active `ai_golden_sets` rows, calls `aiRoute`, scores against `contains`/`regex`/`equals`/`json_has` expectations (safe typed path walker, no `any` casts), writes pass/fail + score + cost + latency to `ai_eval_runs`. Routed at `ai-router/eval-run`.
