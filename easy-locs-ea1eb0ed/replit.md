...
- **ENGINE_MASTER_REGISTRY**: 21 engines with canonical IDs derived directly from engine constructor configs in `engine-registry.ts` → `registerAllEngines()`. IDs match exactly what engines report at runtime (no invented IDs). Includes `data-forex-rates` (ForexDataEngine, 5min interval, auto-fetches ECB/Frankfurter rates) and `data-prayer-times` (PrayerDataEngine, 10min interval, auto-fetches Al-Adhan prayer times with geo fallback).
...
- **Data engines** (`src/engines/data/`): ForexDataEngine auto-fetches forex rates every 5 min and caches in-memory via `getForexEngineCache()`. PrayerDataEngine auto-fetches prayer times every 10 min with geolocation + country fallback via `getPrayerEngineCache()`. Both registered in engine-registry, engine-metadata-registry, and sentinel. Both emit platform bus events (`forex.rates.updated`, `prayer.times.updated`).
