...
- **Orbit Message Dedup**: `buildOptimisticTextMessage` accepts `_uiTempId` to reuse the UI-generated tempId instead of creating a second UUID. `executeSendText` skips store insert when the UI already inserted the optimistic message (prevents double bubbles). Reconciliation works with a single tempId across both layers.
