...
- **Personalized recommendations** (`supabase/functions/ai-recommendations`): builds a user signal vector from `profiles.embedding` (or on-the-fly seed embedding) and queries `semantic_search` across the relevant domain tables. 15-minute cache per (user, domain, context_hash) in `ai_recommendations_cache`. Routed at `ai-router/recommendations`. Client helper: `src/lib/ai/recommendations-client.ts`.
- **Guardrails** (`supabase/functions/_shared/ai-guardrails.ts`): prompt-injection detection, PII redaction (email / phone / IBAN / card / FR NIR / API key), optional OpenAI moderation. Applied to inbound user queries in `ai-rag`; outbound assistant text scrubbed via `sanitizeAssistantOutput`. Client mirror: `src/lib/ai/guardrails.ts`.
- **Cost / latency / quota tracking** (`supabase/functions/_shared/ai-cost-tracker.ts`): `ai_interactions` row per call (provider, model, tokens, cost USD, latency, fallback, block reason). Atomic `ai_quota_increment` RPC rolls into `ai_quotas` per user/day/feature. Defaults: 200 req/day, 200k tokens/day, $2/day (env-overridable: `AI_QUOTA_REQUESTS_PER_DAY`, `AI_QUOTA_TOKENS_PER_DAY`, `AI_QUOTA_COST_PER_DAY`).
- **Automated evals** (`supabase/functions/ai-eval-runner`): service-role runner iterates active `ai_golden_sets` rows, calls `dispatchAiCompletion` (the registry-governed entrypoint — LB Closeout #845 retired the legacy `aiRoute` wrapper), scores against `contains`/`regex`/`equals`/`json_has` expectations (safe typed path walker, no `any` casts), writes pass/fail + score + cost + latency to `ai_eval_runs`. Routed at `ai-router/eval-run`.
- **Content enrichment pipeline** (`supabase/functions/ai-content-enrichment`): single-pass tool-call generates description (150-250w), 5-12 tags, SEO title (≤60c), meta (≤155c), keywords, and per-image alt text (≤120c each), clamped server-side. Supports per-entity (`{entityType, entityId, force?}`) or bulk (`{entityType, limit, missingOnly}`) modes over `listings` / `seed_products` / `marketplace_services`. Inputs pass through guardrails; outputs through `sanitizeAssistantOutput`. Persisted to `content_enrichments` keyed by `(entity_type, entity_id)` with `input_hash` to skip re-work when facts are unchanged. Routed at `ai-router/content-enrichment`. Client helper: `src/lib/ai/content-enrichment-client.ts`.
- **Quota accounting on cache hits**: `ai-recommendations` now calls `logAiInteraction` on cache hits (zero tokens, `metadata.cache="hit"`) so per-user daily request caps stay honest; cache misses record actual embedding tokens + model via `generateEmbedding`'s reported usage.
- **Migration**: `20260419000000_next_gen_ia.sql` creates `ai_interactions`, `ai_quotas`, `ai_conversation_memory`, `ai_golden_sets`, `ai_eval_runs`, `ai_recommendations_cache`, `content_enrichments` (+ `updated_at` trigger), the `listings.search_tsv` generated column + GIN index, and the `hybrid_search_listings` RPC. All tables have RLS: `self_read` for the owning user / authenticated read for enrichments, full access for `service_role`.
- **Barrel**: `src/lib/ai/index.ts` re-exports rag / recommendations / guardrails client helpers.

## Codebase Detox (Task #272)
- **TURN Credentials Server-Side Only**: TURN server credentials are no longer in the client bundle. All ICE config (including guest-call) fetches from the `get-turn-credentials` Supabase Edge Function at runtime. The edge function generates short-lived HMAC-based TURN credentials (TURN REST API pattern) using `TURN_SHARED_SECRET`. Authenticated users get 1h TTL, guests get 5min TTL. Supports Twilio and Metered API providers as alternatives. Webhook mock secrets replaced with empty array in DeliveryAPIWebhooks.tsx.
- **Ephemeral TURN for Guests (Task #410)**: Guest callers on restricted networks (UAE/GCC firewalls) now receive ephemeral TURN credentials when `TURN_PROVIDER` is set to `twilio` or `metered_api`. Edge function returns `turnAvailable` flag; client tracks it via `isTurnAvailable()`. Relay-only retry in `GuestCallManager` skips immediately with `network_blocked` when no TURN servers are available. Explicit `static` provider mode returns STUN-only with server-side warning logs.
...
- **AI Provider**: All AI calls flow through `dispatchAiCompletion` / `dispatchAiEmbedding` / `dispatchAiRag` / `dispatchAiToolUse` in `supabase/functions/_shared/execution/ai-dispatch.ts` — the single sanctioned entrypoint. Provider selection, model, and fallback chain are configured per AI agent in `system.agents.metadata.router`; quota, sensitive-output routing, approval flow, and `ai_interactions` audit are guaranteed by the platform. Direct `api.openai.com` host calls are restricted to the canonical homes (`_shared/ai-router.ts` for chat + moderation, `_shared/execution/adapters/ai/runner-aiRoute.ts` for embeddings); the legacy `_shared/openai-client.ts` is a throw-error stub. The `lambda-handlers/ai-tasks` worker calls the `ai-proxy` Edge Function (which itself dispatches) so even out-of-band Node runtimes inherit the same governance. **LB Closeout #845** retired the legacy `aiRoute()` / `aiRouteAndParse()` env-only wrappers in `_shared/ai-router.ts` — the only remaining in-process primitive is `aiRouteForAgent()` which requires a registry-resolved `AgentRouterConfig`. Static guards: `src/__tests__/track1-bypass-removed.integration.test.ts` (host + symbol scan), `src/__tests__/lb1-track2-migration.contract.test.ts` (per-function callsite contract), and `src/__tests__/lb-closeout-845-aiRoute-retired.test.ts` (export + repo-wide call assertions) prevent the bypass from creeping back in.
- **Analytics**: PostHog (explicit events only, no autocapture/session recording), Google Analytics (opt-in behind cookie consent only), Sentry (error tracking), Segment CDP (`@segment/analytics-next`, lazy init via `VITE_SEGMENT_WRITE_KEY`). Browser telemetry engine disabled (no silent DB writes).
- **Auth**: Supabase Auth native (Google/Apple OAuth). No third-party auth proxies.
- **Super App Instant Performance (Task #1033)**: Closes the remaining gaps on the "perçue instantanée" target, on top of the existing perf stack (Tasks #191, #463, #767).
  - **Stale-While-Revalidate default**: `queryClient` now sets `placeholderData: keepPreviousData` globally. Filter / paginate / sort changes keep the previous list on screen while the new request is in flight — no more "blank flash" on Radar, Marketplace and Wallet history. Per-query opt-out is still possible via `placeholderData: undefined`.
  - **Web Vitals self-hosted ingest path**: `src/lib/performance/web-vitals-reporter.ts` ships an additive collection path. When `VITE_WEB_VITALS_ENDPOINT` is set, every metric is batched (≤10 entries / 5s window) and shipped via `navigator.sendBeacon` (with `keepalive` fetch fallback) to a configurable endpoint — survives `pagehide`, bfcache and `visibilitychange→hidden`. PostHog reporting is preserved unchanged. Endpoint left unset → strict no-op (zero overhead). Covered by `src/lib/performance/__tests__/web-vitals-reporter.test.ts` (4 tests: enqueue, timer flush, max-batch flush, unset env).
...
- **MapLibre Migration (Task #570)**: Map rendering migrated from Mapbox GL to MapLibre GL. Canonical loader at `src/lib/maplibre/maplibre-loader.ts` exports `loadMapLibre()` / `getMapLibreGL()` / `preloadMapLibre()`. Old `src/lib/mapbox/mapbox-loader.ts` re-exports from maplibre for backward compat. Config at `src/lib/maplibre/config.ts` exports `getMapTokenError()`. All map components (LiveMap, ClientMapCard, SellerMapCard, UnifiedMap, SuperMap, MobilityLiveMap, RideLiveMap, RideLiveMapCard, InAppNavigationView, LocationViewerOverlay, DubaiAnalyticsPage) import from `@/lib/maplibre/*`. Hooks (useMapCore, useMapInteractions, useMapCamera) also use maplibre paths. Default style: `https://basemaps.cartocdn.com/gl/dark-matter-gl-style/style.json`. MapErrorBoundary wraps all map entry points with context-specific `fallbackTitle`/`fallbackIcon` props. MapErrorFallback supports `title`, `icon`, retry with backoff, offline indicator, cooldown, exhaustion states. Route-aware LRU dedup cache in `map-error-analytics.ts` (100 entries). Map error analytics persisted to Supabase `map_error_analytics` table with alerting thresholds in `map-error-alerting.ts`. Never use `import mapboxgl from "mapbox-gl"` or `import maplibregl from "maplibre-gl"` directly — always use the loader.
- **Phase 1B Speed Engine (Task #463)**: Worker-powered, streaming-first architecture.
  - **Web Workers + Comlink**: Pool manager (`src/workers/pool-manager.ts`) with `execute()` method — assigns to least-busy worker, tracks busy/taskCount per worker, auto-creates up to `maxWorkersPerType` workers, auto-shrinks idle pools. 4 typed workers: crypto (AES-GCM E2EE → `local-crypto.ts`), search (tokenized index → `radarSearchService.ts`), normalization (phone normalization → `shop-import-pipeline.ts`), analytics-batch (size + 10s interval flush → `journeyTracker.ts`). All callers use `workerPool.execute<API, ReturnType>()` for proper load tracking.
  - **Analytics Pipeline**: Segment CDP is the single destination — journey events are batched in analytics worker, flushed every 10s or on size threshold, then dispatched to Segment. PostHog `captureEvent()` sends to PostHog only (no Segment mirror). `identifyActor()`/`resetPostHog()` dual-dispatch to Segment for identity sync.
  - **Partytown**: PostHog CDN script offloaded as `type="text/partytown"` in `index.html`. Bundled PostHog init skipped when `__POSTHOG_KEY__` is set on window (Partytown path active). Sentry/Firebase use ES module imports (not CDN scripts) — Partytown is not applicable for bundled modules.
  - **Predictive Prefetch**: `src/lib/performance/prefetch-engine.ts` upgraded — tracks 50 transitions in sessionStorage, computes transition probabilities, preloads routes >30% likelihood on idle. Connection-aware (skips on slow-2g/saveData). 10 route modules registered in App.tsx.
  - **React Transitions**: `TransitionRouter` wraps `<Routes>` with `useTransition` + `displayLocation` state. `TransitionRoutes` renders `<Routes location={displayLocation}>` so route rendering is driven by the deferred location. Opacity 0.85 during pending transitions.
  - **SharedWorker Realtime Multiplexing**: `createRealtimeChannel()` returns a channel proxy object. Actual Supabase channel creation deferred to ownership callback — only owner tabs open WebSocket connections. Non-owner tabs receive events via SharedWorker relay through `onCrossTabRealtimeEvent()`. Ownership transfers on tab close. Fallback: creates direct channel when SharedWorker unavailable.
  - **SharedWorker Cross-Tab Sync**: `src/workers/cross-tab-sync.ts` + `cross-tab-client.ts` — multiplexes orbit messages, wallet balance, notification counts across tabs via BroadcastChannel. Heartbeat + stale tab cleanup.
  - **Segment CDP**: `src/lib/analytics/segment.ts` — `@segment/analytics-next` with lazy init (needs `VITE_SEGMENT_WRITE_KEY`). Unified track/identify/page/group. `segmentTrackWithContext()` enriches events with viewport/path/version.
  - **Performance Budgets**: Per-pillar limits (Dashboard:300KB, Radar:350KB, Orbit:300KB, Wallet:250KB, Me:200KB). CI script: `scripts/check-budget.ts` (`npm run check:budget`).
  - **Resource Hints**: preconnect/dns-prefetch in index.html for Stripe, Firebase, PostHog, Segment, Sentry.
  - **Packages**: comlink, @builder.io/partytown, @segment/analytics-next
...
- **Prayer notifications**: `PrayerNotificationProvider` mounted at app bootstrap level (always active). Uses `usePrayerNotifications` hook with dual notification strategy: (1) **In-app**: ServiceWorkerRegistration.showNotification() + adhan audio, checks every 30s, fires once per prayer per day; (2) **Background push**: FCM push via `prayer-push-cron` Edge Function for when app is fully closed. Push scheduler (`src/lib/push/prayer-push-scheduler.ts`) pre-schedules remaining daily prayers via setTimeout + SW showNotification for tab-backgrounded scenarios. Prayer schedule synced to `prayer_push_schedules` table with user timezone for server-side cron delivery. Notification click deep-links to `/dashboard/islamic?tab=prayer`. Push registration UI in PrayerTimesTab shows status (active/prompt/denied). Respects per-prayer prefs from `adhan_notification_prefs` table. Permission requested when user has notifications enabled. Gated behind auth + enabled preference to avoid unnecessary geo/API work.
...
- **Cache Metrics Header**: Metrics endpoint prefers `X-Metrics-Key` header over query param (query param is deprecated). Client hook (`useCacheMetrics`) sends `X-Metrics-Key` header when `VITE_CACHE_METRICS_KEY` is set, otherwise falls back to JWT admin auth.
...
- **Auth model**: the endpoint itself is anonymous. To reach the two backing edge functions it sends `Authorization: Bearer ${HEALTH_CHECK_SECRET}`. Both `health-check` and `integration-health-monitor` recognize this dedicated monitor secret and bypass their router-origin / service-role guards (single-purpose secret, rotates independently from the service role key). When `HEALTH_CHECK_SECRET` is not configured in Vercel, the two edge function probes report `skipped` and the overall status still rolls up from Supabase reachability.
...
- **Env vars used**: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `HEALTH_CHECK_SECRET` (must match the value set on the Supabase project for the `health-check` / `integration-health-monitor` functions), optional overrides `SUPABASE_HEALTH_CHECK_URL`, `SUPABASE_INTEGRATION_HEALTH_URL`. `SUPABASE_SERVICE_ROLE_KEY` is **not** required in Vercel — the monitor-secret bypass exists specifically so the service role key never has to leave the Supabase boundary.
