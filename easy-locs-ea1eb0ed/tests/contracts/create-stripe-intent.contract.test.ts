...
describe.skipIf(!_canRun)("Contract: create-stripe-intent (requires user JWT via getUser)", () => {
  it("rejects anon key as Bearer token with 401 and error", async () => {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/create-stripe-intent`, {
      method: "POST",
      headers: {
