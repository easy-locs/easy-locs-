        ...
        <div className="flex items-end gap-1.5">
          ...
          <div className="flex-1 min-w-0 flex items-end rounded-2xl px-1.5 py-1 bg-background border border-border">
            ...
            {onEmoji && (
              <button
                onClick={onEmoji}
                className="shrink-0 h-8 w-8 flex items-center justify-center rounded-full hover:bg-muted active:scale-90 transition-transform touch-target"
              >
            ...
            <DropdownMenu open={showAttachMenu} onOpenChange={setShowAttachMenu}>
              ...
              <DropdownMenuContent align="start" side="top" className="w-44">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => { setShowAttachMenu(false); haptic("light"); toast.info(orbitLabels.media.contact || "Share a contact"); }}>
            ...
            <textarea
              ...
              disabled={disabled}
              rows={1}
              className="flex-1 min-w-0 bg-transparent border-0 outline-none text-sm px-2 py-2 text-foreground placeholder:text-muted-foreground resize-none overflow-y-auto leading-5 transition-[height] duration-100 ease-out"
              style={{ maxHeight: 200 }}
            />
          </div>
