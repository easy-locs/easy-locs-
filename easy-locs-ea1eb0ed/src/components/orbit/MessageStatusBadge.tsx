...
function Badge({ status, isMe, progress }: Props) {
  ...
  return (
    <span className="inline-flex items-center justify-center w-3.5 h-3.5">
      {status === "sending" && (
        <Clock className="h-2.5 w-2.5 animate-pulse" style={{ color: "hsl(var(--muted-foreground) / 0.4)" }} />
      )}
      {status === "sent" && (
        <Check className="h-3 w-3 animate-[fade-in_120ms_ease-out]" style={{ color: "hsl(var(--muted-foreground) / 0.35)" }} />
      )}
      {status === "delivered" && (
        <CheckCheck className="h-3 w-3 animate-[fade-in_120ms_ease-out]" style={{ color: "hsl(var(--muted-foreground) / 0.35)" }} />
      )}
      {status === "read" && (
        <CheckCheck className="h-3 w-3 animate-[fade-in_120ms_ease-out]" style={{ color: "hsl(var(--primary))" }} />
      )}
      {status === "failed" && (
