...
export default function OrbitSessionManager({ userId }: OrbitSessionManagerProps) {
  ...
  const handleRevoke = async (sessionId: string) => {
    ...
    if (ok) {
      toast.success(t("orbit.session_revoked"));
      await load();
    } else {
      toast.error(t("orbit.revoke_failed"));
    }
    setRevoking(null);
  };
  ...
  const handleRevokeAll = async () => {
    ...
    if (ok) {
      toast.success(t("orbit.all_sessions_revoked"));
    } else {
      toast.error(t("orbit.sessions_revoke_failed"));
    }
    await load();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <Loader2 className="h-5 w-5 animate-spin" style={{ color: "hsl(var(--primary) / 0.4)" }} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl p-3" style={{
        background: otherSessions.length === 0 ? "hsl(var(--hud-success) / 0.06)" : "hsl(var(--primary) / 0.04)",
        border: `1px solid ${otherSessions.length === 0 ? "hsl(var(--hud-success) / 0.15)" : "hsl(var(--primary) / 0.1)"}`,
      }}>
        ...
        <div className="rounded-xl p-3" style={{
          ...
          <div className="space-y-1">
            {suspiciousLogins.map((login) => (
              <p key={login.id} className="text-token-xs" style={{ color: "hsl(var(--muted-foreground))" }}>
      ...
      <div>
        <div className="flex items-center justify-between mb-2.5">
          ...
          {otherSessions.length > 0 && (
            <AlertDialog>
              ...
              <AlertDialogContent style={{ background: "hsl(var(--background))", borderColor: "hsl(var(--border) / 0.15)" }}>
                <AlertDialogHeader>
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel style={{ color: "hsl(var(--foreground))" }}>Annuler</AlertDialogCancel>
        ...
        <div className="space-y-1.5">
          {sessions.map((session) => {
            ...
            return (
              <div key={session.id} className="flex items-center gap-3 p-2.5 rounded-lg" style={{
                background: isCurrent ? "hsl(var(--hud-success) / 0.05)" : "hsl(var(--card) / 0.5)",
                border: `1px solid ${isCurrent ? "hsl(var(--hud-success) / 0.12)" : "hsl(var(--border) / 0.08)"}`,
              }}>
                <div style={{ color: isCurrent ? "hsl(var(--hud-success))" : "hsl(var(--muted-foreground))" }}>
                  {getDeviceIcon(session.os || "")}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                     <span className="text-sm font-medium min-w-0 break-words leading-snug" style={{ color: "hsl(var(--foreground))" }}>
                  ...
                  <p className="text-token-xs" style={{ color: "hsl(var(--muted-foreground) / 0.6)" }}>
          ...
          {sessions.length === 0 && (
            <p className="text-xs text-center py-4" style={{ color: "hsl(var(--muted-foreground) / 0.4)" }}>
