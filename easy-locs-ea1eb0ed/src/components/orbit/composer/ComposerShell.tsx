          ...
          <div className="flex items-end gap-1.5">
            {attachmentActions ? (
              <ComposerAttachMenu actions={attachmentActions} disabled={disabled}>
                <button className="shrink-0 h-8 w-8 touch-target flex items-center justify-center rounded-full hover:bg-muted active:scale-90 transition-transform">
            ...
            <ComposerTextInput
              ...
              disabled={disabled}
            />
            {!hasText && attachmentActions?.onCameraCapture && (
              <button
                onClick={() => {
                  ...
                  inp.onchange = () => {
                    const f = inp.files?.[0];
                    if (f) attachmentActions.onCameraCapture!(f);
                  };
                  inp.click();
                }}
                disabled={disabled}
                className="shrink-0 h-8 w-8 touch-target flex items-center justify-center rounded-full hover:bg-muted active:scale-90 transition-transform"
              >
