...
function MessageCardRenderer({ msg, isMe, currentUserId, onCallBack }: Props) {
  ...
  if (cardType === "payment") {
    const payMeta = envelope.metadata.payment;
    const isTransfer = payMeta?.paymentType === "receipt" &&
      envelope.body?.toLowerCase().includes("transfer");
    if (isTransfer) {
      return <TransferCard envelope={envelope} isMe={isMe} />;
    }
