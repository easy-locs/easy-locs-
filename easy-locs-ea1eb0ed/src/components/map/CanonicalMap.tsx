...
export default memo(function CanonicalMap({
  ...
  return (
    <div className={`relative w-full h-full ${className}`} style={{ minHeight: 300 }}>
      ...
      {!weather.loading && (
        <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-card/85 backdrop-blur-md border border-border/15 shadow-lg z-10 pointer-events-auto">
          <span className="text-base leading-none">{weather.icon}</span>
          <span className="text-[11px] font-semibold text-foreground">{weather.shortLabel}</span>
          {weather.windKmh != null && weather.windKmh > 15 && (
            <span className="text-[10px] text-muted-foreground">💨{Math.round(weather.windKmh)}</span>
          )}
        </div>
      )}
    </div>
