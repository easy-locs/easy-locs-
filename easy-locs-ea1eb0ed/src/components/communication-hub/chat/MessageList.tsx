...
const MessageList = memo(forwardRef<HTMLDivElement, Props>(({
  ...
  const filtered = useMemo(() => {
    return messages.filter(msg => {
      ...
      if (msg.deleted_for_sender && isOutgoingMessage(msg, userId)) return false;
      if (userId && msg.deleted_for_user_ids?.includes(userId)) return false;
      return true;
    });
    ...
    <div ref={ref} className="flex-1 overflow-y-auto overflow-x-hidden px-3 sm:px-4 py-3" style={{ background: "hsl(var(--background))", overscrollBehavior: "contain", willChange: "scroll-position", paddingBottom: "max(env(safe-area-inset-bottom, 24px), 24px)" }}>
        ...
        <div className="flex items-center justify-center h-full">
          <div className="text-center px-6">
            <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{
              background: "hsl(var(--card))", border: "1px solid hsl(var(--border) / 0.08)",
            }}>
              <MessageCircle className="h-7 w-7" style={{ color: "hsl(var(--muted-foreground) / 0.25)" }} />
            </div>
            <p className="text-sm font-medium" style={{ color: "hsl(var(--foreground))" }}>{t("orbit.no_messages")}</p>
            <p className="text-xs mt-1" style={{ color: "hsl(var(--muted-foreground))" }}>{t("orbit.start_conversation")}</p>
          </div>
        </div>
      ) : (
      ...
      {typingIndicator && (
        <div className="flex justify-start mt-2 animate-[bubbleIn_200ms_ease-out_both]">
          <div className="rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-2" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border) / 0.06)" }}>
