...
function MessageBubbleRouterInner({ msg, isMe, attachment, currentUserId, blurred }: Props) {
  ...
  if (isViewOnce && (kind === "image" || kind === "video")) {
    const meta = msg.metadata_json ?? msg.metadata;
    const sourceInput = buildMediaSourceInput(attachment, msg.attachment_url, meta);
    const url = resolveMediaRenderableSource(sourceInput) || msg.attachment_url || "";
    return (
      <div className="mb-1">
        <ViewOnceMedia
          messageId={msg.id}
          attachmentUrl={url}
          isMe={isMe}
          viewOnceOpenedAt={msg.view_once_opened_at ?? null}
          viewOnceOpenedBy={msg.view_once_opened_by ?? null}
  ...
  switch (kind) {
      ...
      return (
        <div className={`mb-1 ${blurred ? "blur-lg transition-all" : ""}`}>
          <VideoBubble
            ...
            isMe={isMe}
            fileName={extractFileName(renderUrl)}
            duration={attachment?.duration ?? msg.video_duration_seconds}
    ...
    case "voice":
      return (
        <VoiceBubble
          src={msg.audio_url || renderUrl}
          durationSeconds={attachment?.duration ?? msg.audio_duration_seconds ?? 0}
          isMe={isMe}
          messageId={msg.id}
          uploadProgress={attachment?.uploadProgress}
          uploadStatus={attachment?.uploadStatus}
