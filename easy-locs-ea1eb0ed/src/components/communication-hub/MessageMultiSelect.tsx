...
export default function MessageMultiSelectToolbar({
  ...
  const handleDeleteForMe = async () => {
    ...
    try {
      ...
      onDeletedForMe(ids);
      toast.success(`${count} ${t("orbit.messages_hidden")}`);
  ...
  const handleDeleteForAll = async () => {
    ...
    try {
      ...
      onDeletedForAll(ids);
      toast.success(`${count} ${t("orbit.messages_deleted_all")}`);
    ...
    <>
      ...
      >
        ...
        <span className="text-sm font-semibold flex-1" style={{ color: "hsl(var(--foreground))" }}>
          {count} {t("orbit.selected")}
        </span>
        <div className="flex items-center gap-1.5">
          <Button variant="ghost" size="icon" onClick={handleCopyAll} className="h-9 w-9 rounded-full" title={t("orbit.copy_text")}>
            <Copy className="h-4 w-4" style={{ color: "hsl(var(--muted-foreground))" }} />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setShowForward(true)} className="h-9 w-9 rounded-full" title={t("orbit.forward")}>
            <Forward className="h-4 w-4" style={{ color: "hsl(var(--muted-foreground))" }} />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setConfirmAction("deleteMe")} className="h-9 w-9 rounded-full" title={t("orbit.delete_for_me")}>
            <EyeOff className="h-4 w-4" style={{ color: "hsl(var(--hud-warning, 40 90% 60%))" }} />
          </Button>
          {allMine && (
            <Button variant="ghost" size="icon" onClick={() => setConfirmAction("deleteAll")} className="h-9 w-9 rounded-full" title={t("orbit.delete_for_all")}>
          ...
          <DialogHeader>
            <DialogTitle>
              {confirmAction === "deleteMe"
                ? (t("orbit.delete_for_me_q"))
                : (t("orbit.delete_for_all_q"))}
            </DialogTitle>
            <DialogDescription>
              {confirmAction === "deleteMe"
                ? `${count} ${t("orbit.delete_for_me_desc")}`
                : `${count} ${t("orbit.delete_for_all_desc")}`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            >
              {processing ? (t("orbit.deleting")) : confirmAction === "deleteMe" ? (t("orbit.hide")) : (t("orbit.delete"))}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
