    ...
    <div className="w-72 lg:w-80 border-s border-border/50 flex flex-col overflow-hidden bg-muted/5">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
              <User className="h-3.5 w-3.5 text-accent" /> {t("orbit.context.contact")}
            </h3>
          ...
          {thread.conversationType === "property" && (
            <>
              {(thread.propertyLabel || thread.propertyCountry) && (
                <div className="space-y-1 p-2.5 rounded-lg bg-muted/30 border border-border/30">
                  <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                    <Home className="h-3 w-3 text-primary" /> {t("orbit.context.property")}
                  </h4>
              ...
              {propertyCtx.loading ? (
                ...
                <>
                  {propertyCtx.leases.length > 0 && (
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        <FileText className="h-3 w-3 text-accent" /> {t("orbit.context.lease")}
                      </h4>
                  ...
                  {propertyCtx.rentCalls.length > 0 && (
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        <CreditCard className="h-3 w-3 text-accent" /> {t("orbit.context.rent")}
                      </h4>
                  ...
                  {propertyCtx.interventions.length > 0 && (
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        <Wrench className="h-3 w-3 text-amber-500" /> {t("orbit.context.maintenance")}
                      </h4>
                  ...
                  {propertyCtx.documents.length > 0 && (
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        <FileText className="h-3 w-3 text-blue-500" /> {t("orbit.context.documents")}
                      </h4>
              ...
              <div className="space-y-1">
                ...
                <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 gap-1.5" asChild>
                  <a href={`/dashboard/documents`}><FileText className="h-3 w-3" /> {t("orbit.context.documents")}</a>
                </Button>
              </div>
            </>
          )}
            ...
            <>
              {bookingCtx.loading ? (
                ...
                <>
                  <div className="space-y-2 p-2.5 rounded-lg bg-muted/30 border border-border/30">
                    <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                      <Calendar className="h-3 w-3 text-accent" /> {t("orbit.context.booking")}
                    </h4>
                    <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {bookingCtx.booking && (
                      <div className="space-y-1 text-xs text-muted-foreground pt-1 border-t border-border/20">
                        ...
                        {bookingCtx.booking.payment_status && (
                          <p>💳 {t("orbit.context.payment")}: <span className={bookingCtx.booking.payment_status === "paid" ? "text-emerald-600 font-medium" : "text-amber-600 font-medium"}>{bookingCtx.booking.payment_status}</span></p>
                        )}
                        {bookingCtx.booking.quantity && bookingCtx.booking.quantity > 1 && <p>👥 {t("orbit.context.qty")}: {bookingCtx.booking.quantity}</p>}
                      </div>
                    )}
                  </div>
                  ...
                  {bookingCtx.service && (
                    <div className="space-y-1.5 p-2.5 rounded-lg bg-muted/20 border border-border/20">
                      <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        <Building className="h-3 w-3 text-accent" /> {t("orbit.context.service")}
                      </h4>
              ...
              <div className="space-y-1">
                {thread.bookingType === "marketplace" && (
                  <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 gap-1.5" asChild>
                    <a href={`/dashboard/my-shop`}><ExternalLink className="h-3 w-3" /> {t("orbit.context.view_in_shop")}</a>
                  </Button>
                )}
                {thread.bookingType === "concierge" && (
                  <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 gap-1.5" asChild>
                    <a href={`/dashboard/activities?booking=${thread.bookingId}`}><ExternalLink className="h-3 w-3" /> {t("orbit.context.view_in_marketplace")}</a>
                  </Button>
                )}
                {thread.bookingType === "seasonal" && (
                  <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 gap-1.5" asChild>
                    <a href={`/dashboard/seasonal?booking=${thread.bookingId}`}><ExternalLink className="h-3 w-3" /> {t("orbit.context.view_in_seasonal")}</a>
                  </Button>
                )}
              </div>
            </>
          ...
          {thread.conversationType === "listing" && (
            <div className="space-y-2 p-2.5 rounded-lg bg-muted/30 border border-border/30">
              ...
              <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 gap-1.5" asChild>
                <a href="/dashboard/real-estate"><ExternalLink className="h-3 w-3" /> {t("orbit.context.view_listings")}</a>
              </Button>
            </div>
          )}
          ...
          {thread.conversationType !== "property" && (thread.propertyLabel || thread.propertyCountry) && (
            <div className="space-y-1 p-2.5 rounded-lg bg-muted/20 border border-border/20">
              <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                <Building className="h-3 w-3 text-accent" /> {t("orbit.context.property")}
              </h4>
              {thread.propertyCountry && (
                <span className="text-xs">{getCountryEntryOrDefault(thread.propertyCountry).flag} {getCountryEntryOrDefault(thread.propertyCountry).name}</span>
              )}
      ...
      <div className="flex-shrink-0 border-t border-border/30 flex flex-col" style={{ maxHeight: "35%" }}>
        <div className="px-4 py-2.5 flex items-center gap-1.5">
          ...
          <h4 className="text-xs font-semibold text-foreground">{t("orbit.context.activity_timeline")}</h4>
        </div>
