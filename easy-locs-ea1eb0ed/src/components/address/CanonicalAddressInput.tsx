  ...
  return (
    <div ref={wrapperRef} className={cn("relative", className)}>
      ...
      <div className="relative min-w-0">
        {!hideSearchIcon && (
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 shrink-0 text-muted-foreground/60 pointer-events-none" />
        )}
        <input
          ref={inputRef}
          type="text"
          ...
          className={cn(
            "w-full min-w-0 h-12 pr-10 rounded-xl border border-border/30 bg-card text-sm leading-normal text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all",
            hideSearchIcon ? "pl-4" : "pl-11"
          )}
        />
        {(value || query) && (
