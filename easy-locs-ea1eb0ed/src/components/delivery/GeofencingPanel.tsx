    ...
    <div className={`space-y-3 ${className || ""}`}>
      ...
      {showAdd && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
          ...
          <Label className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim))" }}>Nom de zone</Label>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Label className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Lat</Label>
            ...
            <div>
              <Label className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Lng</Label>
              <Input type="number" value={newZone.lng} step={0.001}
                onChange={e => setNewZone(p => ({ ...p, lng: Number(e.target.value) }))}
                className="h-6 text-[10px]" style={{ background: "hsl(var(--hud-border) / 0.04)" }} />
            </div>
            <div>
              <Label className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Rayon km</Label>
        ...
        {zones.length === 0 ? (
          ...
          return (
            <motion.div key={zone.id} layout
              ...
              <button onClick={() => toggleZone(zone.id)} className="text-[10px] px-2 py-0.5 rounded"
      ...
      {driverPos && zones.filter(z => z.active).length > 0 && (
        <div className="rounded-xl p-3"
          ...
          {zones.filter(z => z.active && z.type === "restricted" && isInZone(z)).map(z => (
            <p key={z.id} className="text-[10px] py-0.5" style={{ color: "hsl(var(--destructive))" }}>
              ⚠️ Vous êtes dans la zone restreinte "{z.name}"
            </p>
          ))}
          {zones.filter(z => z.active && z.type === "allowed" && !isInZone(z)).map(z => (
            <p key={z.id} className="text-[10px] py-0.5" style={{ color: "hsl(var(--warning))" }}>
          ...
          {zones.filter(z => z.active).every(z => {
            ...
            <p className="text-[10px]" style={{ color: "hsl(var(--success))" }}>✅ Toutes les zones OK</p>
          )}
        </div>
      )}
    </div>
