  ...
  return (
    <div className={`space-y-3 ${className || ""}`}>
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" onClick={() => setShowAdd(!showAdd)} className="text-[0.625rem] h-7">
      ...
      {showAdd && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
          ...
          <Label className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim))" }}>Nom de zone</Label>
          <Input value={newZone.name} onChange={e => setNewZone(p => ({ ...p, name: e.target.value }))}
            placeholder="Ex: Centre-ville" className="h-7 text-[0.6875rem]" style={{ background: "hsl(var(--hud-border) / 0.04)" }} />
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Label className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Lat</Label>
              <Input type="number" value={newZone.lat} step={0.001}
                onChange={e => setNewZone(p => ({ ...p, lat: Number(e.target.value) }))}
                className="h-6 text-[0.625rem]" style={{ background: "hsl(var(--hud-border) / 0.04)" }} />
            </div>
            <div>
              <Label className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Lng</Label>
              <Input type="number" value={newZone.lng} step={0.001}
                onChange={e => setNewZone(p => ({ ...p, lng: Number(e.target.value) }))}
                className="h-6 text-[0.625rem]" style={{ background: "hsl(var(--hud-border) / 0.04)" }} />
            </div>
            <div>
              <Label className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Rayon km</Label>
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => setShowAdd(false)} className="text-[0.625rem] h-7 flex-1">Annuler</Button>
            <Button size="sm" onClick={addZone} className="text-[0.625rem] h-7 flex-1"
      ...
      <div className="space-y-1.5">
        {zones.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.3)" }}>Aucune zone configurée</p>
          </div>
        ) : zones.map(zone => {
          ...
          return (
            <motion.div key={zone.id} layout
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[0.6875rem] font-semibold truncate" style={{ color: "hsl(var(--hud-text))" }}>{zone.name}</p>
              ...
              <button onClick={() => toggleZone(zone.id)} className="text-[0.625rem] px-2 py-0.5 rounded"
      ...
      {driverPos && zones.filter(z => z.active).length > 0 && (
        <div className="rounded-xl p-3"
          ...
          <p className="text-[0.625rem] font-semibold mb-1.5" style={{ color: "hsl(var(--hud-text-dim))" }}>
            <AlertTriangle className="h-3 w-3 inline mr-1" /> Alertes de zone
          </p>
          {zones.filter(z => z.active && z.type === "restricted" && isInZone(z)).map(z => (
            <p key={z.id} className="text-[0.625rem] py-0.5" style={{ color: "hsl(var(--destructive))" }}>
              ⚠️ Vous êtes dans la zone restreinte "{z.name}"
            </p>
          ))}
          {zones.filter(z => z.active && z.type === "allowed" && !isInZone(z)).map(z => (
            <p key={z.id} className="text-[0.625rem] py-0.5" style={{ color: "hsl(var(--warning))" }}>
          ...
          {zones.filter(z => z.active).every(z => {
            ...
            <p className="text-[0.625rem]" style={{ color: "hsl(var(--success))" }}>✅ Toutes les zones OK</p>
          )}
        </div>
      )}
    </div>
