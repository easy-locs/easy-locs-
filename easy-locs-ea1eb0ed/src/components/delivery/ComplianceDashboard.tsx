              ...
              return (
                <div key={c.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>{c.details}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-[10px] font-semibold" style={{ color: cfg.color }}>{cfg.label}</span>
                    {c.affectedCount != null && c.affectedCount > 0 && (
                      <p className="text-[10px]" style={{ color: cfg.color }}>{c.affectedCount} concerné{c.affectedCount > 1 ? "s" : ""}</p>
                    )}
                  </div>
                </div>
              );
                ...
                <div key={d.id} className="rounded-xl p-3 space-y-2" style={{ background: "hsl(var(--hud-surface))", border: `1px solid ${allGood ? "hsl(var(--success) / 0.12)" : "hsl(var(--warning) / 0.12)"}` }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>Dernière vérif: {d.lastVerified}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-4 gap-1">
                    {[
                      ...
                      <div key={item.label} className="text-center py-1 rounded-md" style={{ background: "hsl(var(--hud-bg))" }}>
                        <p className="text-[10px]">{item.ok ? "✅" : d.backgroundCheck === "pending" && item.label === "Casier" ? "⏳" : "❌"}</p>
                        <p className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>{item.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              return (
                <div key={a.id} className="px-3 py-2 rounded-xl" style={{ background: cfg.bg, border: `1px solid ${cfg.color}15` }}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold" style={{ color: cfg.color }}>{a.actor}</span>
                  </div>
                  <p className="text-[10px] font-semibold mt-0.5" style={{ color: "hsl(var(--hud-text))" }}>{a.action}</p>
                  <p className="text-[10px]" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>Cible: {a.target}</p>
                </div>
              );
            })}
