  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[0.625rem] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "hsl(var(--hud-cyan) / 0.12)", color: "hsl(var(--hud-cyan))" }}>
          🧠 Gemini
        </span>
      </div>
        ...
        {tab === "forecast" && (
          <motion.div key="forecast" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
              <p className="text-[0.625rem] font-semibold mb-2" style={{ color: "hsl(var(--hud-text-dim))" }}>Demande prévue (24h)</p>
                ...
                {forecast.map(f => {
                  ...
                  return (
                    <div key={f.hour} className="flex-1 flex flex-col items-center justify-end h-full relative group">
                      ...
                      />
                      {f.hour % 4 === 0 && (
                        <span className="text-[0.625rem] mt-1 absolute -bottom-3" style={{ color: "hsl(var(--hud-text-dim) / 0.3)" }}>{f.hour}h</span>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="flex items-center gap-4 mt-4">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>Prévu</span>
                </div>
              </div>
            </div>

            <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
              <p className="text-[0.625rem] font-semibold mb-1" style={{ color: "hsl(var(--hud-text-dim))" }}>🎯 Confiance du modèle</p>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[0.6875rem] font-bold" style={{ color: accuracy > 85 ? "hsl(var(--success))" : "hsl(var(--warning))" }}>{accuracy}%</span>
              </div>
            </div>
          </motion.div>
        )}
                ...
                <div key={z.id} className="rounded-xl p-3 space-y-2" style={{ background: "hsl(var(--hud-surface))", border: `1px solid ${DEMAND_COLORS[z.predictedDemand]}15` }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-[0.6875rem] font-bold" style={{ color: "hsl(var(--hud-text))" }}>{z.name}</p>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="text-center py-1 rounded-lg" style={{ background: "hsl(var(--hud-bg))" }}>
                      <p className="text-[0.625rem] font-bold" style={{ color: "hsl(var(--hud-text))" }}>{z.currentDrivers}</p>
                      <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Présents</p>
                    </div>
                    <div className="text-center py-1 rounded-lg" style={{ background: "hsl(var(--hud-bg))" }}>
                      <p className="text-[0.625rem] font-bold" style={{ color: "hsl(var(--hud-cyan))" }}>{z.recommendedDrivers}</p>
                      <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Recommandés</p>
                    </div>
                    <div className="text-center py-1 rounded-lg" style={{ background: deficit > 0 ? "hsl(var(--destructive) / 0.06)" : "hsl(var(--success) / 0.06)" }}>
                      <p className="text-[0.625rem] font-bold" style={{ color: deficit > 0 ? "hsl(var(--destructive))" : "hsl(var(--success))" }}>
                        {deficit > 0 ? `+${deficit}` : "✓"}
                      </p>
                      <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>Besoin</p>
                    </div>
                  </div>
                </div>
              );
                ...
                <div key={s.id} className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: `1px solid ${cfg.color}15` }}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-[0.6875rem] font-bold" style={{ color: "hsl(var(--hud-text))" }}>{s.title}</p>
                        <span className="text-[0.625rem] font-semibold px-1.5 py-0.5 rounded-full shrink-0" style={{ background: `${cfg.color}12`, color: cfg.color }}>{cfg.label}</span>
                      </div>
                      <p className="text-[0.625rem] mt-1" style={{ color: "hsl(var(--hud-text-dim) / 0.5)" }}>{s.description}</p>
                      {s.actionable && (
                        <button className="mt-2 text-[0.625rem] font-semibold px-2 py-1 rounded-md" style={{ background: "hsl(var(--hud-cyan) / 0.12)", color: "hsl(var(--hud-cyan))" }}>
                          ⚡ Appliquer
                        </button>
                      )}
                    </div>
