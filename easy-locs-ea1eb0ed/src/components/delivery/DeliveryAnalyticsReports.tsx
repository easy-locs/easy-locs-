  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          {(["7d", "30d", "90d"] as Period[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className="px-3 py-1 rounded-lg text-[0.625rem] font-semibold transition-all"
              style={{
        ...
        <Button size="sm" variant="outline" className="text-[0.625rem] h-7 px-2.5"
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
            <p className="text-[0.625rem] font-semibold mb-2" style={{ color: "hsl(var(--hud-text-dim))" }}>⚡ Records de vitesse</p>
            <div className="flex items-center justify-around">
              <div className="text-center">
                <p className="text-lg font-bold" style={{ color: "hsl(var(--success))" }}>{analytics.fastestMin}</p>
                <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>min. (le plus rapide)</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>min. (moyenne)</p>
              </div>
              <div className="h-8 w-px" style={{ background: "hsl(var(--hud-border) / 0.1)" }} />
              <div className="text-center">
                <p className="text-lg font-bold" style={{ color: "hsl(var(--destructive))" }}>{analytics.slowestMin}</p>
                <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>min. (le plus lent)</p>
              </div>
            </div>
          </div>
          ...
          {/* Priority breakdown */}
          <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
            <p className="text-[0.625rem] font-semibold mb-2" style={{ color: "hsl(var(--hud-text-dim))" }}>📦 Par priorité</p>
            <div className="flex gap-2">
              {[
                ...
                <div key={p.label} className="flex-1 text-center py-2 rounded-lg"
                  ...
                  <p className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text-dim) / 0.4)" }}>{p.label}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
              ...
              <div className="flex justify-between mt-1">
                ...
                <div className="flex gap-3">
                  <span className="text-[0.625rem] flex items-center gap-0.5" style={{ color: "hsl(var(--success) / 0.6)" }}>
                    <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: "hsl(var(--success) / 0.6)" }} /> Succès
                  </span>
                  <span className="text-[0.625rem] flex items-center gap-0.5" style={{ color: "hsl(var(--destructive) / 0.4)" }}>
          ...
          {/* Heatmap summary */}
          <div className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: "1px solid hsl(var(--hud-border) / 0.08)" }}>
            <p className="text-[0.625rem] font-semibold mb-1" style={{ color: "hsl(var(--hud-text-dim))" }}>
            ...
            {analytics.heatPoints.length > 0 && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-[0.625rem]" style={{ color: "hsl(var(--hud-text))" }}>
