          ...
          {ZONES.map(z => (
            <div key={z.name} className="rounded-xl p-3 flex items-center gap-3"
              ...
              <div className="text-right">
                ...
                {z.alerts > 0 && (
                  <span className="text-[10px] font-bold" style={{ color: "hsl(var(--destructive))" }}>⚠ {z.alerts}</span>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="rounded-xl p-3"
              ...
              <p className="text-[10px]" style={{ color: "hsl(var(--muted-foreground))" }}>{m.issue}</p>
              <Button size="sm" className="mt-2 text-[10px] h-6 w-full" onClick={() => { haptic("medium"); }}
