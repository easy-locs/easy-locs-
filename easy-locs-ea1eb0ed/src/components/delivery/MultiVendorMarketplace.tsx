    ...
    <div className={`space-y-3 ${className || ""}`}>
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setView("cart"); haptic("selection"); }}
          ...
          {totalItems > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-[0.625rem] font-bold flex items-center justify-center"
          ...
          <div className="flex gap-1 overflow-x-auto pb-1">
            {categories.map(c => (
              <button key={c} onClick={() => { setCategoryFilter(c); haptic("selection"); }}
                className="px-2 py-1 rounded-full text-[0.625rem] font-semibold whitespace-nowrap shrink-0"
                style={{
          ...
          {filtered.map(p => (
            <div key={p.id} className="rounded-xl p-3 flex items-center gap-3"
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[0.625rem] font-semibold truncate" style={{ color: "hsl(var(--foreground))" }}>{p.name}</p>
                <p className="text-[0.625rem]" style={{ color: "hsl(var(--muted-foreground))" }}>
                  {p.seller} • ⭐ {p.rating} • 📍 {p.zone}
                </p>
                <p className="text-[0.625rem] font-bold mt-0.5" style={{ color: "hsl(var(--primary))" }}>{p.price.toLocaleString()} F</p>
              </div>
              <Button size="sm" className="text-[0.625rem] h-7 px-2" onClick={() => addToCart(p)}
        ...
        <div className="space-y-2">
          {cart.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-[0.625rem]" style={{ color: "hsl(var(--muted-foreground))" }}>Panier vide</p>
            </div>
          ) : (
            <>
              {sellerGroups.map(seller => (
                <div key={seller} className="space-y-1.5">
                  <p className="text-[0.625rem] font-bold flex items-center gap-1" style={{ color: "hsl(var(--primary))" }}>
                  ...
                  {cart.filter(c => c.seller === seller).map(c => (
                    <div key={c.id} className="rounded-lg p-2 flex items-center gap-2"
                      ...
                      <span>{c.image}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-[0.625rem] font-medium truncate" style={{ color: "hsl(var(--foreground))" }}>{c.name}</p>
                        <p className="text-[0.625rem]" style={{ color: "hsl(var(--muted-foreground))" }}>{c.price.toLocaleString()} F</p>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[0.625rem] font-bold w-4 text-center" style={{ color: "hsl(var(--foreground))" }}>{c.qty}</span>
                        <button onClick={() => updateQty(c.id, 1)} className="w-5 h-5 rounded flex items-center justify-center"
                          style={{ background: "hsl(var(--primary) / 0.1)" }}>
                          <Plus className="h-3 w-3" style={{ color: "hsl(var(--primary))" }} />
                        </button>
                      </div>
                      <span className="text-[0.625rem] font-bold w-14 text-right" style={{ color: "hsl(var(--foreground))" }}>
              ...
              <div className="rounded-xl p-3 mt-2" style={{ background: "hsl(var(--primary) / 0.05)", border: "1px solid hsl(var(--primary) / 0.15)" }}>
                <div className="flex justify-between">
                  <span className="text-[0.6875rem] font-bold" style={{ color: "hsl(var(--foreground))" }}>Total</span>
                  <span className="text-[0.6875rem] font-bold" style={{ color: "hsl(var(--primary))" }}>{cartTotal.toLocaleString()} F</span>
                </div>
            ...
            return (
              <div key={sid} className="rounded-xl p-3"
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[0.625rem] font-semibold" style={{ color: "hsl(var(--foreground))" }}>{seller.seller}</span>
                ...
                <div className="flex items-center gap-1 mt-1">
                  <span className="text-[0.625rem] font-semibold" style={{ color: "hsl(var(--success))" }}>🚚 ~25 min</span>
                  <span className="text-[0.625rem]" style={{ color: "hsl(var(--muted-foreground))" }}>•</span>
                  <span className="text-[0.625rem] font-semibold" style={{ color: "hsl(var(--warning))" }}>500 F livraison</span>
                </div>
              </div>
            );
          })}
