      ...
      <div className="grid grid-cols-2 gap-2">
        {kpis.map(k => (
          <div key={k.label} className="rounded-xl p-3" style={{ background: "hsl(var(--hud-surface))", border: `1px solid ${k.color}15` }}>
            ...
            {k.target && (
              <p className="text-[10px] mt-1" style={{ color: "hsl(var(--hud-text-dim) / 0.3)" }}>🎯 Objectif: {k.target}</p>
            )}
          </div>
        ))}
      </div>
            ...
            {dailyMetrics.length === 0 ? (
              ...
              return (
                <div key={d.date} className="rounded-lg px-3 py-2 flex items-center gap-3"
                  ...
                  <span className="text-[10px] font-bold w-12 text-right" style={{ color: rate >= 90 ? "hsl(var(--success))" : "hsl(var(--warning))" }}>{rate}%</span>
                  <span className="text-[10px] w-6 text-right" style={{ color: "hsl(var(--hud-text-dim) / 0.3)" }}>{d.deliveries}</span>
                </div>
              );
            })}
          </motion.div>
