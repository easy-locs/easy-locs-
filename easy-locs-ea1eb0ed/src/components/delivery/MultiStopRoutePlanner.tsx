...
import { haversineKm, estimateETA } from "@/lib/geo/distance";
...
export default function MultiStopRoutePlanner({ orgId }: { orgId: string }) {
  ...
  const addStop = () => {
    ...
    const prevLat = lastStop?.lat ?? 48.8566;
    const prevLng = lastStop?.lng ?? 2.3522;
    const newLat = prevLat + (stops.length % 2 === 0 ? 0.008 : -0.005);
    const newLng = prevLng + 0.006;
    const dist = +(haversineKm(prevLat, prevLng, newLat, newLng)).toFixed(1);
    const time = Math.round(estimateETA(dist));
    setStops(prev => [...prev, {
      id: `s${Date.now()}`, address: newAddress, type: newType,
      estimatedMinutes: time, distanceKm: dist, lat: newLat, lng: newLng,
    }]);
    setNewAddress("");
    setOptimized(false);
  };
  ...
  const optimizeRoute = () => {
    ...
    let cumDist = 0;
    let prevLat = pickups[0]?.lat ?? 48.8566;
    let prevLng = pickups[0]?.lng ?? 2.3522;
    const optimizedDropoffs = dropoffs.map((d) => {
      const dLat = d.lat ?? prevLat;
      const dLng = d.lng ?? prevLng;
      const leg = haversineKm(prevLat, prevLng, dLat, dLng);
      cumDist += leg;
      prevLat = dLat;
      prevLng = dLng;
      return { ...d, distanceKm: +cumDist.toFixed(1), estimatedMinutes: Math.round(estimateETA(cumDist)) };
    });
