...
export default function DeliveryNotificationCenter({ orgId, className }: Props) {
  ...
  const markRead = async (id: string) => {
    haptic("light");
    await db("app_notifications").update({ read_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", id);
  ...
  const markAllRead = async () => {
    ...
    await db("app_notifications").update({ read_at: new Date().toISOString(), updated_at: new Date().toISOString() }).in("id", ids);
  ...
  const deleteNotif = async (id: string) => {
    haptic("light");
    await db("app_notifications").update({ dismissed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", id);
