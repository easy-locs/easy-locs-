...
export default function MultiCurrencyTax({ shopId, mode }: Props) {
  ...
  const { data: rules = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("storefront_tax_rules").select("*").eq("shop_id", shopId).order("country");
      return data || [];
    },
    enabled: mode === "seller",
  });
  ...
  const { data: invoices = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("storefront_invoices").select("*").eq("shop_id", shopId).order("created_at", { ascending: false }).limit(50);
      return data || [];
    },
  });
  ...
  const deleteRule = useMutation({
    mutationFn: async (id: string) => {
      await db("storefront_tax_rules").delete().eq("id", id);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["tax-rules"] });
