...
export default function AICategorySuggest({ shopId, onAccept }: Props) {
  ...
  const { data: verticals = [] } = useQuery({
    queryKey: ["verticals"],
    queryFn: async () => {
      const { data } = await db("verticals").select("slug, name").eq("active", true).order("sort_order");
      return data || [];
    },
  });
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("categories").select("slug, name, vertical_id, verticals(slug)").eq("active", true).order("sort_order");
      return data || [];
    },
  });
  ...
  const handleSuggest = async () => {
    ...
    try {
      ...
      // Save suggestion
      if (user) {
        await db("ai_category_suggestions").insert({
          shop_id: shopId || null,
          user_id: user.id,
