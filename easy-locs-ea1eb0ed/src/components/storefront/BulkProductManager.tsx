...
export default function BulkProductManager({ shopId }: Props) {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("catalog_items").select("*").eq("shop_id", shopId).order("sort_order");
      return data || [];
    },
    enabled: !!shopId,
  });
  ...
  const bulkUpdate = useMutation({
    mutationFn: async (updates: Record<string, any>) => {
      ...
      for (const id of ids) {
        await db("catalog_items").update({ ...updates, updated_at: new Date().toISOString() }).eq("id", id);
      }
    },
  ...
  const updatePrice = useMutation({
    mutationFn: async ({ id, price }: { id: string; price: number }) => {
      await db("catalog_items").update({ price, updated_at: new Date().toISOString() }).eq("id", id);
    },
