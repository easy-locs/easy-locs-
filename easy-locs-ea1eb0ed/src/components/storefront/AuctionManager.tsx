...
export default function AuctionManager({ shopId, mode, catalogItems = [] }: Props) {
  ...
  const { data: auctions = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const q = db("storefront_auctions").select("*").eq("shop_id", shopId).order("created_at", { ascending: false });
  ...
  const createAuction = async () => {
    ...
    const { error } = await db("storefront_auctions").insert({
      shop_id: shopId,
  ...
  const placeBid = async (auctionId: string) => {
    ...
    const { error } = await db("storefront_auction_bids").insert({
