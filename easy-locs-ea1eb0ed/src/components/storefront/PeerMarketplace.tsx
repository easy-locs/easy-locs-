...
export default function PeerMarketplace({ shopId, mode = "buyer" }: Props) {
  ...
  const { data: listings = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("storefront_p2p_listings")
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data } = await db("storefront_p2p_transactions")
  ...
  const createListing = async () => {
    ...
    setSaving(true);
    try {
      await db("storefront_p2p_listings").insert({
  ...
  const initiatePurchase = async (listing: any) => {
    ...
    await db("storefront_p2p_transactions").insert({
  ...
  const updateTransaction = async (txId: string, updates: Record<string, any>) => {
    await db("storefront_p2p_transactions").update(updates).eq("id", txId);
    qc.invalidateQueries({ queryKey: ["p2p-tx", shopId, user?.id] });
                  ...
                  <CardContent className="p-2.5 space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={`text-[10px] ${escrowStatusColor(tx.escrow_status)}`}>{tx.escrow_status}</Badge>
                      </div>
                    </div>
