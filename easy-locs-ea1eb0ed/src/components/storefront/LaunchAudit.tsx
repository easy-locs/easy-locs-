...
export default function LaunchAudit({ shopId }: Props) {
  ...
  const { data: audit, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("orbit_launch_audits")
  ...
  const runAudit = async () => {
    ...
    try {
      // Check catalog
      const { data: items } = await db("catalog_items").select("id").eq("shop_id", shopId).limit(1);
      const catalog_ready = (items?.length || 0) > 0;

      // Check shop config
      const { data: shop } = await db("storefront_pages").select("*").eq("id", shopId).single();
      ...
      // Check translations
      const { data: translations } = await db("storefront_translations").select("id").eq("shop_id", shopId).limit(1);
      ...
      await db("orbit_launch_audits").upsert({
        shop_id: shopId,
        user_id: user.id,
