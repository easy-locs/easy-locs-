...
export default function BusinessHierarchy({ onBranchShopLink }: Props) {
  ...
  const { data: companies = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("companies").select("*").eq("org_id", orgId).order("created_at");
      return data || [];
    },
    enabled: !!orgId,
  });
  ...
  const { data: brands = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("brands").select("*").eq("org_id", orgId).order("created_at");
      return data || [];
    },
    enabled: !!orgId,
  });
  ...
  const { data: branches = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await db("branches").select("*").eq("org_id", orgId).order("created_at");
      return data || [];
    },
    enabled: !!orgId,
  });
  ...
  const handleAdd = async () => {
    ...
    try {
      if (addDialog === "company") {
        await db("companies").insert({ org_id: orgId, user_id: user.id, name: name.trim() });
        qc.invalidateQueries({ queryKey: ["companies", orgId] });
      } else if (addDialog === "brand" && parentId) {
        await db("brands").insert({ company_id: parentId, org_id: orgId, user_id: user.id, name: name.trim() });
        qc.invalidateQueries({ queryKey: ["brands", orgId] });
      } else if (addDialog === "branch" && parentId) {
        await db("branches").insert({ brand_id: parentId, org_id: orgId, user_id: user.id, name: name.trim() });
  ...
  const handleDelete = async (table: string, id: string) => {
    await db(table).delete().eq("id", id);
