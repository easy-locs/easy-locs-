...
const usePricingFeatures = (t: (key: string) => string) => {
  const FREE_FEATURES = [
    ...
    t("pricing.f.free.7") || "0% fees for clients — 5% platform only",
  ];
  const SOLO_FEATURES = [
    ...
    t("pricing.f.solo.3") || "Only 5% commission — keep 95% of earnings",
...
const TRUST_SIGNALS_DATA = [
  { icon: <BadgePercent className="h-5 w-5" />, labelKey: "pricing.trust.commission", labelFb: "Only 5% commission", subKey: "pricing.trust.commission_sub", subFb: "vs 30% on Uber, Deliveroo, Booking" },
