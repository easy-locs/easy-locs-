 ...
 * Branded "Easy-Locs®" on ONE LINE in the QR center.
 */
...
/* ── Branded QR Logo Overlay — single line ── */
...
export function ScanQrButton({
  ...
  return (
    <Button size={size} variant={variant} className={className} onClick={() => navigate("/pay/scan")}>
      <ScanLine className="h-4 w-4 mr-1.5" />{label}
    </Button>
  );
}

/* ── My QR Code Card — consistent sizing ── */
  ...
  return (
    <div className={`flex flex-col items-center gap-3 ${compact ? "p-3" : "p-5"} rounded-2xl border border-border bg-card w-full`}>
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="flex items-center gap-1.5 rounded-xl bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary transition hover:bg-primary/20 active:scale-[0.97]"
      >
  ...
  return (
    <div className={className}>
      <div className="flex gap-2">
        <Button size="sm" variant="outline" className="flex-1 gap-1.5 active:scale-[0.97]" onClick={() => navigate("/pay/scan")}>
          <ScanLine className="h-4 w-4" /> Scan
        </Button>
        <Button size="sm" variant="outline" className="flex-1 gap-1.5 active:scale-[0.97]" onClick={() => setShowQr(!showQr)}>
