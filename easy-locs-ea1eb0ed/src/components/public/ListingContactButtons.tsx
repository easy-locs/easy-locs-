...
async function getOrCreateV2Conversation(userId: string, orgId: string, opts: {
  ...
  try {
    ...
    const participants = [
      ...
      ...(org?.owner_user_id ? [{ userId: org.owner_user_id, orbitId: `orbit_${org.owner_user_id.replace(/-/g, "").substring(0, 8)}` }] : []),
    ];
...
const ListingContactButtons = ({
  ...
  const freeCallLockRef = useRef(false);
  const handleFreeCall = async () => {
    if (!user || !orgId) return;
    if (freeCallLockRef.current || isInCall || isStartingCall) return;
    freeCallLockRef.current = true;
    ...
    try {
      ...
      if (!success) {
        toast.error(t("page.listing.call_failed") || "Call could not be started");
      }
    } catch {
      toast.error(t("page.listing.call_failed") || "Call failed");
    } finally {
      setTimeout(() => { freeCallLockRef.current = false; }, 500);
    }
  };
