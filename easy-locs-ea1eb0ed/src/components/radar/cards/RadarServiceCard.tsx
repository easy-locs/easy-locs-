...
function RadarServiceCard({ item, rank, selected, onSelect, onNavigate, onMessage }: Props) {
  ...
  return (
    <div
      ...
      style={{
        background: selected ? "hsl(270 60% 55% / 0.05)" : "hsl(var(--card))",
        borderColor: selected ? "hsl(270 60% 55% / 0.25)" : "hsl(var(--border) / 0.1)",
      }}
    >
      {rank != null && (
        <div className="w-5 flex items-center justify-center shrink-0">
          <span className="text-[11px] font-extrabold tabular-nums" style={{ color: rank <= 3 ? "hsl(270 60% 55%)" : "hsl(var(--muted-foreground))" }}>
            {rank}
          </span>
        </div>
      )}
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          {item.type === "healthcare" && (
            <span className="px-1 py-0.5 rounded text-[9px] font-bold" style={{ background: "hsl(0 65% 50% / 0.1)", color: "hsl(0 65% 50%)" }}>
              HC
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {item.priceLabel && (
            <span className="text-[10px] font-semibold text-muted-foreground">{item.priceLabel}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {onNavigate && (
          <button onClick={e => { e.stopPropagation(); haptic("light"); onNavigate(); }} aria-label={`Navigate to ${item.title}`}
            className="w-8 h-8 rounded-xl flex items-center justify-center active:scale-90 transition-transform"
            style={{ background: "hsl(270 60% 55% / 0.1)" }}>
            <Navigation className="w-3.5 h-3.5" style={{ color: "hsl(270 60% 55%)" }} />
          </button>
        )}
      </div>
    </div>
