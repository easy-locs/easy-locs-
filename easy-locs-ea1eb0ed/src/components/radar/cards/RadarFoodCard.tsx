...
function RadarFoodCard({ item, rank, selected, onSelect, onNavigate, onMessage }: Props) {
  ...
  const cuisineLabel = item.subcategory
  ...
  return (
    <div
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl border transition-all active:scale-[0.98] cursor-pointer ${
        selected ? "bg-accent/5 border-accent/25 ring-1 ring-accent/10" : "bg-card border-border/10"
      }`}
    >
      {rank != null && (
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          {cuisineLabel && <span className="text-xs text-muted-foreground line-clamp-1">{cuisineLabel}</span>}
          {item.priceLabel && <span className="text-xs font-semibold text-muted-foreground">{item.priceLabel}</span>}
        </div>
