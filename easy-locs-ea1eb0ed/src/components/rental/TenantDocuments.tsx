...
const TenantDocuments = ({ tenantId, tenantName }: Props) => {
  ...
  const handleRequestDocument = async (docType: string, label: string) => {
    ...
    try {
      ...
      await tdRepo.insertChatMessage({ conversation_id: contextId, sender_user_id: user.id, sender_orbit_id: `orbit_${user.id.replace(/-/g, "").substring(0, 8)}`, type: "text", body: t("comp.docs.doc_requested_msg").replace("{label}", label) });
        ...
        await tdRepo.invokeSendEmail({
          ...
          html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#ffffff;">
            ...
            <div style="text-align:center;margin:24px 0;">
              <a href="${appUrl}/dashboard/documents" style="display:inline-block;background:#d4a853;color:#1a1a1a;font-weight:600;text-decoration:none;padding:12px 32px;border-radius:8px;font-size:15px;">${t("comp.docs.email_cta")}</a>
            </div>
    ...
    try {
      ...
      await tdRepo.invokeSendEmail({
        ...
        html: `<div style="font-family:sans-serif;max-width:700px;margin:0 auto;padding:24px;background:#ffffff;">
          ...
          <div style="text-align:center;margin:24px 0;">
            <a href="${appUrl}/dashboard/documents" style="display:inline-block;background:#d4a853;color:#1a1a1a;font-weight:600;text-decoration:none;padding:12px 32px;border-radius:8px;font-size:15px;">${t("comp.docs.email_docs_cta")}</a>
          </div>
          <p style="color:#888;font-size:12px;text-align:center;">${t("hook.rental.receipt_email_footer")}</p>
        </div>`,
      });
