...
export type RiderSnapPoint = "peek" | "half" | "full";

const SNAP_HEIGHTS: Record<RiderSnapPoint, string> = {
  peek: "240px",
  half: "55vh",
  full: "85vh",
};
...
interface Props {
  snap: RiderSnapPoint;
  onSnapChange: (snap: RiderSnapPoint) => void;
  children: ReactNode;
  className?: string;
}

export default function RiderBottomSheet({ snap, onSnapChange, children, className }: Props) {
    ...
    >
      ...
      >
        <button
          ...
          aria-label={`${snap === "peek" ? "Expand" : snap === "half" ? "Expand fully" : "Collapse"} driver panel`}
