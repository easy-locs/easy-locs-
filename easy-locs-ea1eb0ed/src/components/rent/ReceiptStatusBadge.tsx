...
export default function ReceiptStatusBadge({ receiptUrl, validated, paid, className }: ReceiptStatusBadgeProps) {
  ...
  if (!receiptUrl) {
    return (
      <Badge variant="outline" className={`gap-1 text-[0.625rem] text-muted-foreground ${className || ""}`}>
  ...
  if (validated) {
    return (
      <Badge variant="outline" className={`gap-1 text-[0.625rem] text-success border-success/30 ${className || ""}`}>
  ...
  return (
    <Badge variant="outline" className={`gap-1 text-[0.625rem] text-primary border-primary/30 ${className || ""}`}>
