...
async function handleInvoice(booking: any, service: any, provider: any) {
  ...
  try {
    ...
    toast.error("Invoice upload failed — the invoice will be sent without attachment.");
  }
...
async function shareInvoiceEmail(booking: any, service: any, provider: any, navigateFn?: (path: string) => void) {
  ...
  const path = await navigateToOrbitThread({ targetEmail: booking.booker_email, targetName: booking.booker_name || "Client" });
