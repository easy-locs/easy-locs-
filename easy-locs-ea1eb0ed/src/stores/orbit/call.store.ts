...
export const useCallStore = create<CallStoreState>((set, get) => ({
  ...
  transition: (state) => {
    ...
    const CALL_TRANSITIONS: Record<string, string[]> = {
      ...
      incoming: ["connecting", "declined", "missed", "ended"],
  ...
  toggleSpeaker: () => {
    ...
    const audioEl = document.querySelector<HTMLAudioElement>("audio[autoplay]");
    if (audioEl) {
      void CallMediaEngine.setOutput(newSpeaker ? "speaker" : "earpiece", audioEl);
    }
  ...
  endCall: (finalState = "ended") => {
    ...
    if (TERMINAL_STATES.includes(call.uiState)) return;
