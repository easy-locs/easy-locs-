...
export async function insertAnalyticsEvent(payload: Record<string, any>) {
  await db("storefront_analytics_events").insert(payload);
}
...
export async function upsertCartItem(
  ...
  if (existingId) {
    await db("storefront_cart_items").update({ quantity }).eq("id", existingId);
  } else {
    await db("storefront_cart_items").insert({
...
export async function deleteCartItem(cartItemId: string) {
  await db("storefront_cart_items").delete().eq("id", cartItemId);
}
...
export async function updateCartItemQuantity(cartItemId: string, quantity: number) {
  if (quantity <= 0) {
    ...
    await db("storefront_cart_items").update({ quantity }).eq("id", cartItemId);
  }
}

export async function clearCartItems(cartId: string) {
  await db("storefront_cart_items").delete().eq("cart_id", cartId);
}
...
export async function recordCouponUsage(couponId: string, userId: string, orderId: string | null, discountAmount: number) {
  await db("storefront_coupon_usage").insert({
