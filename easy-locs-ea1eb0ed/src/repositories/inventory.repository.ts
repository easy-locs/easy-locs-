...
export async function insertInventoryReport(record: Record<string, any>) {
  const { data, error } = await db("inventory_reports").insert(record).select("id").single();
...
export async function insertRoom(record: Record<string, any>) {
  const { data, error } = await db("inventory_rooms").insert(record).select("id").single();
  if (error) throw error;
  return data.id;
}

export async function insertItems(items: Record<string, any>[]) {
  const { error } = await db("inventory_items").insert(items);
  if (error) throw error;
}
