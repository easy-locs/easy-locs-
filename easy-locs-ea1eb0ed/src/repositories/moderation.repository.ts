...
export async function fetchModerationData() {
  const [driversRes, disputesRes] = await Promise.all([
    cFrom("rider_presence").select("*").limit(100),
    cFrom("delivery_disputes").select("*").order("created_at", { ascending: false }).limit(50),
  ]);
...
export async function fetchDriverNames(userIds: string[]) {
  ...
  const { data } = await cFrom("profiles").select("id, name, first_name, last_name").in("id", userIds);
  return data || [];
}

export async function suspendDriver(userId: string) {
  await cFrom("rider_presence").update({ is_online: false, is_available: false }).eq("user_id", userId);
}

export async function insertModerationAuditLog(userId: string, orgId: string, action: string, metadata: Record<string, any>) {
  await cFrom("audit_logs").insert({ user_id: userId, org_id: orgId, action, metadata_json: metadata });
}

export async function insertModerationNotification(notification: Record<string, any>) {
  await cFrom("app_notifications").insert(notification);
}

export async function resolveDispute(disputeId: string, resolution: string) {
  await cFrom("delivery_disputes").update({
