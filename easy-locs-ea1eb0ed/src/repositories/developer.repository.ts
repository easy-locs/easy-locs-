...
export async function fetchApiKeys(orgId: string) {
  const { data } = await cFrom("api_keys").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
...
export async function fetchWebhooks(orgId: string) {
  const { data } = await cFrom("webhooks").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
  return (data || []) as unknown as Array<{
...
export async function fetchWebhookDeliveries() {
  const { data } = await cFrom("webhook_deliveries").select("*").order("delivered_at", { ascending: false }).limit(50);
  return (data || []) as unknown as Array<{
...
export async function createApiKey(orgId: string, name: string) {
  const { data, error } = await cRpc("create_api_key", {
...
export async function deleteApiKey(id: string) {
  const { error } = await cFrom("api_keys").delete().eq("id", id);
  if (error) throw error;
}

export async function createWebhook(orgId: string, userId: string, url: string, events: string[]) {
  const { error } = await cFrom("webhooks").insert({
...
export async function deleteWebhook(id: string) {
  const { error } = await cFrom("webhooks").delete().eq("id", id);
  if (error) throw error;
}

export async function toggleWebhook(id: string, active: boolean) {
  const { error } = await cFrom("webhooks").update({ active }).eq("id", id);
  if (error) throw error;
}
