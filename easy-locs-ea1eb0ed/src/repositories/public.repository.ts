...
export async function fetchPublicMarketplaceServices(params: Record<string, any>) {
  const { data } = await cRpc("get_public_marketplace_services" as any, params);
  return data ?? [];
}

export async function fetchPublicRealEstateListings(params: Record<string, any>) {
  const { data } = await cRpc("get_public_real_estate_listings" as any, params);
  return data ?? [];
}

export async function fetchPublicListings(filters?: Record<string, any>) {
  let q = cFrom("public_listings").select("*").eq("active", true);
...
export async function fetchPublicProviders(params: Record<string, any>) {
  const { data } = await cRpc("get_public_marketplace_providers" as any, params);
  return data ?? [];
}
...
export async function insertBookingRequest(record: Record<string, any>) {
  const { error } = await cFrom("booking_requests").insert(record);
  if (error) throw error;
}

// ── Listing contact ──
export async function fetchListingContact(listingId: string) {
  const { data } = await cFrom("public_listings").select("property_id, org_id").eq("id", listingId).single();
  return data;
}
...
export async function resolveQrCode(code: string) {
  const { data } = await cFrom("qr_codes").select("*").eq("code", code).single();
  return data;
}
...
export async function fetchSavedListings(userId: string): Promise<SavedListingRow[]> {
  const { data } = await cFrom("saved_listings").select("*").eq("user_id", userId);
  return (data ?? []) as SavedListingRow[];
}

export async function insertSavedListing(record: Record<string, any>) {
  const { error } = await cFrom("saved_listings").insert(record);
  if (error) throw error;
}

export async function deleteSavedListing(userId: string, listingId: string) {
  const { error } = await cFrom("saved_listings").delete().eq("user_id", userId).eq("listing_id", listingId);
  if (error) throw error;
}
...
export async function fetchReferralStats(userId: string) {
  const { data } = await cFrom("referral_rewards").select("*").eq("referrer_user_id", userId);
  return data ?? [];
}

// ── Merchant claim ──
export async function claimMerchant(merchantId: string, userId: string) {
  const { error } = await cFrom("auto_discovered_merchants").update({ claimed_by: userId, claimed_at: new Date().toISOString(), claim_status: "claimed" }).eq("id", merchantId);
  if (error) throw error;
}

export async function fetchSeedMerchant() {
  const { data } = await cFrom("seed_merchants").select("id").limit(1).maybeSingle();
  return data;
}

// ── Merchant orders ──
export async function fetchMerchantOrders(shopId: string) {
  const { data } = await cFrom("storefront_orders").select("*").eq("shop_id", shopId).order("created_at", { ascending: false }).limit(200);
  return data ?? [];
}

export async function fetchOrderItems(limit = 5000) {
  const { data } = await cFrom("order_items").select("*").limit(limit);
  return data ?? [];
}
