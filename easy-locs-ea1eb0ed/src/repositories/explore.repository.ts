...
export async function getPublicRealEstateListingsFiltered(params: Record<string, any>) {
  const { data, error } = await db.rpc("get_public_real_estate_listings", params);
  return { data: data || [], error };
}

export async function getListingProperties(propertyIds: string[]) {
  const { data } = await db.rpc("get_public_listing_properties", { p_property_ids: propertyIds });
  return data || [];
}

export async function getListingProperty(listingId: string) {
  const { data } = await db.rpc("get_listing_property", { p_listing_id: listingId });
  return data;
}

export async function getRealEstateShowcase(slug: string) {
  const { data } = await db.rpc("get_real_estate_showcase", { p_slug: slug });
  return data || [];
}

export async function insertRealEstateLead(payload: Record<string, any>) {
  const { data } = await db("real_estate_leads").insert(payload).select("id").single();
  return data;
}
