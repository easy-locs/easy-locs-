...
export async function fetchProperties(orgId: string, countryFilter?: string | null) {
  let q = cFrom("properties").select("*").eq("org_id", orgId);
...
export async function upsertProperty(orgId: string, userId: string, record: Record<string, any>, editId?: string) {
  ...
  if (editId) {
    const { error } = await cFrom("properties").update(payload as any).eq("id", editId);
    if (error) throw error;
  } else {
    const { error } = await cFrom("properties").insert(payload as any);
    if (error) throw error;
  }
}
...
export async function fetchTenants(orgId: string) {
  const { data } = await cFrom("tenants").select("*").eq("org_id", orgId).order("name");
  return data ?? [];
}
...
export async function upsertTenant(orgId: string, userId: string, record: Record<string, any>, editId?: string) {
  const payload = { org_id: orgId, user_id: userId, ...record };
  if (editId) {
    const { error } = await cFrom("tenants").update(payload as any).eq("id", editId);
    if (error) throw error;
    return editId;
  } else {
    const { data, error } = await cFrom("tenants").insert(payload as any).select("id").single();
    if (error) throw error;
    return data.id;
  }
}
...
export async function fetchRentCalls(orgId: string) {
  const { data } = await cFrom("rent_calls").select("*").eq("org_id", orgId).order("month", { ascending: false });
  return data ?? [];
}

export async function fetchExistingRentCallsForMonth(orgId: string, month: string) {
  const { data } = await cFrom("rent_calls").select("tenant_id").eq("org_id", orgId).eq("month", month);
  return data ?? [];
}

export async function insertRentCalls(calls: Record<string, any>[]) {
  const { error } = await cFrom("rent_calls").upsert(calls as any, { onConflict: "org_id,tenant_id,month", ignoreDuplicates: true });
  if (error) throw error;
}

export async function updateRentCall(id: string, update: Record<string, any>) {
  const { error } = await cFrom("rent_calls").update(update).eq("id", id);
  if (error) throw error;
}

export async function insertSingleRentCall(record: Record<string, any>) {
  const { data, error } = await cFrom("rent_calls").insert(record as any).select("id").single();
  if (error) throw error;
  return data;
}
...
export async function fetchProfile(userId: string) {
  const { data } = await cFrom("profiles").select("name, email, signature_url").eq("id", userId).single();
  return data;
}

export async function fetchOwnerProfile(orgId: string) {
  const { data } = await cFrom("owner_profiles").select("full_name, address, postal_code, city").eq("org_id", orgId).limit(1).maybeSingle();
  return data;
}

export async function fetchOrgInfo(orgId: string) {
  const { data } = await cFrom("orgs").select("stamp_url, name, address, postal_code, city").eq("id", orgId).single();
  return data;
}

// ─── Documents ───
export async function insertDocument(record: Record<string, any>) {
  const { error } = await cFrom("documents").insert(record as any);
  if (error) throw error;
}
...
export async function insertTenantInvitation(record: Record<string, any>) {
  const { error } = await cFrom("tenant_invitations").insert(record as any);
  if (error) throw error;
}

export async function fetchPropertyCountry(propertyId: string) {
  const { data } = await cFrom("properties").select("country").eq("id", propertyId).single();
  return data?.country ?? "FR";
}

// ─── Tenant Assignment ───
export async function updateTenantProperty(tenantId: string, propertyId: string) {
  const { error } = await cFrom("tenants").update({ property_id: propertyId } as any).eq("id", tenantId);
  if (error) throw error;
}
