...
export async function convertDealToOrder(deal: any, shopId: string) {
  ...
  const { data: order } = await db("storefront_orders").insert({
    shop_id: shopId,
...
export async function createDigitalProduct(shopId: string, userId: string, form: any) {
  await db("storefront_digital_products").insert({
    shop_id: shopId,
    user_id: userId,
    title: form.title,
...
export async function purchaseDigitalProduct(productId: string, buyerId: string, shopId: string, downloadLimit: number) {
  await db("storefront_digital_purchases").insert({
...
export async function incrementProductSales(productId: string, currentSales: number) {
  await db("storefront_digital_products").update({
...
export async function incrementDownloadCount(purchaseId: string, currentCount: number) {
  await db("storefront_digital_purchases").update({
...
export async function updateReturnStatus(returnId: string, status: string, notes?: string) {
  ...
  await db("storefront_returns").update(updates).eq("id", returnId);
}

// ── Catalog ──
export async function fetchCatalogProducts(shopId: string) {
  const { data } = await db("storefront_products").select("*").eq("shop_id", shopId).order("sort_order");
  return data ?? [];
}

export async function insertProduct(record: Record<string, any>) {
  const { data, error } = await db("storefront_products").insert(record).select("*").single();
  if (error) throw error;
  return data;
}

export async function updateProduct(id: string, updates: Record<string, any>) {
  const { error } = await db("storefront_products").update(updates).eq("id", id);
  if (error) throw error;
}

export async function deleteProduct(id: string) {
  const { error } = await db("storefront_products").delete().eq("id", id);
  if (error) throw error;
}

// ── Shop creation ──
export async function insertShop(record: Record<string, any>) {
  const { data, error } = await db("storefront_pages").insert(record).select("*").single();
  if (error) throw error;
  return data;
}
...
export async function insertCategorySuggestion(record: Record<string, any>) {
  await db("ai_category_suggestions").insert(record);
}

// ── Delivery dispatch ──
export async function fetchShopOrders(shopId: string) {
  const { data } = await db("storefront_orders").select("*").eq("shop_id", shopId).order("created_at", { ascending: false }).limit(100);
  return data ?? [];
}

export async function updateOrder(orderId: string, updates: Record<string, any>) {
  const { error } = await db("storefront_orders").update(updates).eq("id", orderId);
  if (error) throw error;
}

// ── Smart builders ──
export async function fetchShopsByUser(userId: string) {
  const { data } = await db("storefront_pages").select("*").eq("owner_user_id", userId);
  return data ?? [];
}
...
export async function insertCatalogItem(payload: Record<string, any>) {
  await db("catalog_items").insert(payload);
}

export async function updateCatalogItem(id: string, payload: Record<string, any>) {
  await db("catalog_items").update(payload).eq("id", id);
}

export async function deleteCatalogItem(id: string) {
  await db("catalog_items").delete().eq("id", id);
}
