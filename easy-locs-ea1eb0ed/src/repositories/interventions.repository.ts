...
export async function fetchInterventions(orgId: string, propIds?: string[]) {
  let q = cFrom("interventions").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
...
export async function fetchTenantsForOrg(orgId: string) {
  const { data } = await cFrom("tenants").select("id, name, property_id").eq("org_id", orgId).order("name");
  return data || [];
}
...
export async function updateIntervention(id: string, record: Record<string, any>) {
  const { error } = await cFrom("interventions").update(record).eq("id", id);
  if (error) throw error;
}
