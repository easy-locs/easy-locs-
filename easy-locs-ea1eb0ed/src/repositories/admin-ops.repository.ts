...
export async function fetchSuperDashboardMerchants() {
  const { data, error } = await db("seed_merchants").select("id,is_active,is_open,promo_active").limit(1000);
  if (error) throw error;
  return data ?? [];
}

export async function fetchSuperDashboardDrivers() {
  const { data, error } = await db("driver_profiles").select("id,is_online,is_available").limit(1000);
  if (error) throw error;
  return data ?? [];
}

export async function fetchSuperDashboardTickets() {
  const { data, error } = await db("support_tickets").select("id,status").limit(1000);
  if (error) throw error;
  return data ?? [];
}
...
export async function fetchOwnerCockpitStats() {
  ...
  const results = await Promise.all(
    queries.map(q => db(q.table).select("*", { count: "exact", head: true }).catch(() => ({ count: 0 })))
  );
