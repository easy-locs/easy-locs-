...
export async function fetchProfileForDoc(userId: string) {
  const { data } = await db("profiles").select("signature_url, name, email").eq("id", userId).single();
  return data;
}

export async function fetchOwnerProfileForDoc(orgId: string) {
  const { data } = await db("owner_profiles").select("*").eq("org_id", orgId).limit(1).single();
  return data;
}

export async function fetchOrgForDoc(orgId: string) {
  const { data } = await db("orgs")
...
export async function fetchTenantProfile(tenantUserId: string) {
  const { data } = await db("profiles").select("signature_url, id_number").eq("id", tenantUserId).single();
  return data;
}

export async function insertDocument(doc: Record<string, any>) {
  const { error } = await db("documents").insert(doc as any);
