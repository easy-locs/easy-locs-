...
export function installMerchantAutomation(): () => void {
  ...
  unsubs.push(
    platformBus.on("marketplace:listing_published", async (event) => {
      const p = event.payload as MerchantEventPayload;
      if (p?.action === "imported") {
        const launched = await isShopLaunched(p.shopId ?? "");
        if (!launched) {
          structuredLogger.debug("marketplace", "automation_skip", `Merchant imported: ${p.merchantId} — NOT launched, skipping outreach`);
          return;
        }
        structuredLogger.info("marketplace", "automation_outreach", `Merchant imported: ${p.merchantId} — scheduling outreach`);
      }
    })
  );
  ...
  unsubs.push(
    platformBus.on("marketplace:booking_confirmed", (event) => {
      const p = event.payload as MerchantEventPayload;
      if (p?.action === "claimed") {
        structuredLogger.info("marketplace", "automation_claimed", `Merchant claimed: ${p.merchantId} — stopping outreach`);
      }
    })
  );
  ...
  unsubs.push(
    platformBus.on("marketplace:provider_went_live", async (event) => {
      ...
      if (p?.action === "activated") {
        const launched = await isShopLaunched(p.shopId ?? "");
        if (!launched) {
          structuredLogger.debug("marketplace", "automation_skip", `Merchant activated: ${p.merchantId} — NOT launched, skipping welcome`);
          return;
        }
        structuredLogger.info("marketplace", "automation_welcome", `Merchant activated: ${p.merchantId} — sending welcome`);
      }
    })
  );
  ...
  unsubs.push(
    platformBus.on("storefront:order_placed", (event) => {
      const p = event.payload as MerchantEventPayload;
      structuredLogger.info("marketplace", "automation_order", `Order created: ${p.orderId} for merchant ${p.merchantId}`);
    })
  );
