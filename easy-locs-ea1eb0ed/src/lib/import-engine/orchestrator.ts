...
import { evaluateBatchScrapeDecisions } from "./scrape-decision-gate";
...
export function runImportEngine(
  ...
  // Step 0: Scrape Decision Gate (pre-import validation)
  ...
  const matches = detectDuplicates(filteredRecords);
  const groups = groupByDuplicates(filteredRecords, matches);
  steps.push(dedupTimer.finish(true, { input: filteredRecords.length, output: groups.length }));
