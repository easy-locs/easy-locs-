...
const PUBLISH_REQUIREMENTS: Record<string, string[]> = {
  food: ["canonicalName", "address", "lat", "lng", "phone"],
  grocery: ["canonicalName", "address", "lat", "lng"],
  stay: ["canonicalName", "address", "lat", "lng"],
...
function detectIncoherences(entity: CanonicalEntity): string[] {
  ...
  if (entity.vertical === "stay" && entity.photos.length === 0) {
    issues.push("No photos for stay entity");
  }
