...
import { getPendingMessages, dequeueMessage, markSending, markFailed, purgeDeadLetters, pruneStaleCache } from "@/lib/orbit-offline-queue";
...
class BackgroundSyncEngine {
  ...
  private maintenanceIntervalMs = 5 * 60 * 1000; // 5 min
  ...
  async triggerSync(reason: string = "manual"): Promise<void> {
    ...
    try {
      ...
      // 3. Periodic maintenance (dead-letter purge + cache prune)
      const now = Date.now();
      if (now - this.lastMaintenanceAt > this.maintenanceIntervalMs) {
        this.lastMaintenanceAt = now;
        await Promise.allSettled([purgeDeadLetters(), pruneStaleCache()]);
      }
    } finally {
