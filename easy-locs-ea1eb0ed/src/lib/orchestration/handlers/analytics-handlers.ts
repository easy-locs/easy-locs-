...
export function installAnalyticsHandlers(): void {
  platformBus.on("USER_OPEN_HOME", async (event) => {
    ...
    await logOrchestrationEvent({ eventType: "orch_user_open_home", entityId: p.userId ?? "anonymous", entityType: "user_session", metadata: p as Record<string, unknown> });
  });

  platformBus.on("USER_SEARCH", async (event) => {
    const p = event.payload as UserSearchPayload;
    await logOrchestrationEvent({ eventType: "orch_user_search", entityId: p.userId ?? "anonymous", entityType: "search", metadata: p as Record<string, unknown> });
  });
}
