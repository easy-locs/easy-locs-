...
export function installOrderHandlers(): void {
  platformBus.on("ORDER_CREATED", async (event) => {
    ...
    await createNotification({ actorType: "pro", actorId: p.merchantId, templateKey: "merchant_new_order", payload: p as Record<string, unknown> });
    await logOrchestrationEvent({ eventType: "orch_order_created", entityId: p.orderId, entityType: "order", metadata: p as Record<string, unknown> });
  });
  ...
  platformBus.on("PAYMENT_SUCCESS", async (event) => {
    ...
    await createNotification({ actorType: "pro", actorId: p.merchantWalletId, templateKey: "payment_received_pending_fulfillment", payload: p as Record<string, unknown> });
    await logOrchestrationEvent({ eventType: "orch_payment_success", entityId: p.orderId, entityType: "order", metadata: p as Record<string, unknown>, newValue: p.amount });
  });
