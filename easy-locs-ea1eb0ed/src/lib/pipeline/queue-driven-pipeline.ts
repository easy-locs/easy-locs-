...
export async function enqueueEntity(
  ...
  const { data } = await db("entity_pipeline_queue").insert({
    entity_id: entityId,
...
export async function enqueueBatch(
  ...
  const { data } = await db("entity_pipeline_queue").insert(rows).select("id");
  ...
  try {
    switch (stage) {
      ...
      case "normalize": {
        ...
        const { data: entity } = await db("seed_merchants").select("vertical").eq("id", entityId).single();
      ...
      case "score": {
        ...
        const { data: shop } = await db("seed_merchants").select("*").eq("id", entityId).single();
        if (shop) {
...
async function processQueueItem(item: any): Promise<{ success: boolean; stage: string; error?: string }> {
  ...
  if (result.error) {
    ...
    await db("entity_pipeline_queue").update({
      status,
      retries,
