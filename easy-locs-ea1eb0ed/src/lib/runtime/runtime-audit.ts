...
async function checkDispatchTables(): Promise<RuntimeAuditCheck> {
  try {
    const jobs = await db("mobility_jobs").select("id", { head: true, count: "exact" }).limit(1);
    if (jobs.error) return fail("Dispatch tables", "dispatch_tables", jobs.error.message);
    const offers = await db("mobility_job_offers").select("id", { head: true, count: "exact" }).limit(1);
...
async function checkWalletTables(): Promise<RuntimeAuditCheck> {
  try {
    ...
    const accounts = await db("wallet_accounts").select("id", { head: true, count: "exact" }).limit(1);
    if (accounts.error) return fail("Wallet tables", "wallet_tables", accounts.error.message);
    const tx = await db("unified_wallet_transactions").select("id", { head: true, count: "exact" }).limit(1);
    if (tx.error) return fail("Wallet tables", "wallet_tables", tx.error.message);
    const ledger = await db("wallet_ledger_entries").select("id", { head: true, count: "exact" }).limit(1);
...
function checkHashRoutes(): RuntimeAuditCheck {
  try {
    ...
    return rootHasRouter
      ? pass("Router mode", "router_mode", "BrowserRouter active (main.tsx)")
      : warn("Router mode", "router_mode", "Router not detected");
  } catch (e: any) {
