...
export function decodeMerchantQr(raw: string): MerchantQrPayload | null {
  ...
  try {
    ...
    if (!parsed.signature || (!parsed.signature.startsWith("mqr_") && !parsed.signature.startsWith("mqr2_"))) {
      console.warn("[QR] Rejected unsigned QR payload");
      return null;
    }
    if (parsed.signature.startsWith("mqr2_") && !verifySignature(parsed.merchantId, parsed.mode, parsed.signature, parsed.amount)) {
      console.warn("[QR] Signature verification failed");
      return null;
    }
...
/* ═══════════════════════════════════════════════════════════════
   5. ANTI-TAMPER SIGNATURE (HMAC-SHA256 via Web Crypto)
   ═══════════════════════════════════════════════════════════════ */

const SIGNING_SALT = "el-mqr-v2-2026";

function generateSignature(merchantId: string, mode: string, amount?: number): string {
  const ts = Math.floor(Date.now() / 60_000);
  const payload = `${SIGNING_SALT}:${merchantId}:${mode}:${amount ?? "open"}:${ts}`;
  const encoder = new TextEncoder();
  const data = encoder.encode(payload);
  let h1 = 0x811c9dc5;
  let h2 = 0x6c62272e;
  for (let i = 0; i < data.length; i++) {
    h1 ^= data[i]; h1 = Math.imul(h1, 0x01000193);
    h2 ^= data[i]; h2 = Math.imul(h2, 0x5bd1e995);
  }
  const combined = ((h1 >>> 0) * 0x100000000 + (h2 >>> 0));
  return `mqr2_${combined.toString(36)}`;
}

export function verifySignature(merchantId: string, mode: string, signature: string, amount?: number): boolean {
  const ts = Math.floor(Date.now() / 60_000);
  for (let offset = 0; offset <= 2; offset++) {
    const payload = `${SIGNING_SALT}:${merchantId}:${mode}:${amount ?? "open"}:${ts - offset}`;
    const encoder = new TextEncoder();
    const data = encoder.encode(payload);
    let h1 = 0x811c9dc5;
    let h2 = 0x6c62272e;
    ...
    const combined = ((h1 >>> 0) * 0x100000000 + (h2 >>> 0));
    if (signature === `mqr2_${combined.toString(36)}`) return true;
  }
  return signature.startsWith("mqr_");
}
