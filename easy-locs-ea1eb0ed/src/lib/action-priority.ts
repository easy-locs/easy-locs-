...
export function getPrimaryActionForEntity(ctx: EntityContext): PrimaryCTA {
  switch (ctx.entityType) {
    ...
    case "property_buy":
      return { action: "chat", label: "Contact", icon: "chat" };

    case "property_rent":
      return { action: "chat", label: "Contact", icon: "chat" };

    case "property_project":
      return { action: "open", label: "Explore", icon: "open" };
...
export function getSecondaryActions(ctx: EntityContext, primary: PrimaryCTA): PrimaryCTA[] {
  ...
  if (ctx.entityType === "property_buy" || ctx.entityType === "property_rent") {
    if (primary.action !== "open") all.push({ action: "open", label: "View", icon: "open" });
    if (primary.action !== "chat") all.push({ action: "chat", label: "Message", icon: "chat" });
  }

  if (ctx.entityType === "property_project") {
    if (primary.action !== "open") all.push({ action: "open", label: "Explore", icon: "open" });
    if (primary.action !== "chat") all.push({ action: "chat", label: "Contact Developer", icon: "chat" });
  }

  if (ctx.entityType === "stay") {
    if (primary.action !== "open") all.push({ action: "open", label: "View", icon: "open" });
    if (primary.action !== "chat") all.push({ action: "chat", label: "Message Host", icon: "chat" });
  }
