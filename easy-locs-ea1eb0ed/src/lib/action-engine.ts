...
function handleOpen(input: UniversalActionInput, navigate: NavigateFunction): ActionResult {
  ...
  switch (entityType) {
    ...
    case "property_buy":
    case "property_rent":
      if (slug) navigate(`/property/${slug}`);
      return { ok: !!slug };
    case "property_project":
      if (slug) navigate(`/property/project/${slug}`);
      return { ok: !!slug };
    case "stay":
      if (slug) navigate(`/stay/${slug}`);
      return { ok: !!slug };
    default:
...
async function handleChat(
  ...
  if ((input.entityType === "property_buy" || input.entityType === "property_rent") && !targetUserId && input.slug) {
    ...
    if (listing?.user_id) {
      ...
      if (!targetName || targetName === "User") targetName = listing.title || "Agent";
    }
  }
  ...
  if (input.entityType === "stay" && !targetUserId && input.slug) {
    ...
    if (listing?.org_id) {
      ...
      if (org?.owner_id) {
        ...
        if (!targetName || targetName === "User") targetName = org.name || listing.title || "Host";
      }
    }
  }
  ...
  if (input.entityType === "property_project" && !targetUserId && input.slug) {
    ...
    if (listing?.user_id) {
      targetUserId = listing.user_id;
      if (!targetName || targetName === "User") targetName = listing.title || "Developer";
    }
  }
