...
async function kdfRootKey(
  ...
  const derived = new Uint8Array(
    await crypto.subtle.deriveBits(
      { name: "HKDF", hash: HKDF_HASH, salt: new ArrayBuffer(64), info: RATCHET_INFO },
      hkdfKey,
      512
    ) as ArrayBuffer
  );
...
async function kdfChainKey(
  ...
  const ckDerived = new Uint8Array(
    await crypto.subtle.deriveBits(
      { name: "HKDF", hash: HKDF_HASH, salt: new ArrayBuffer(64), info: CHAIN_KEY_INFO },
      ckMaterial,
      256
    ) as ArrayBuffer
  );
  ...
  const mkDerived = new Uint8Array(
    await crypto.subtle.deriveBits(
      { name: "HKDF", hash: HKDF_HASH, salt: new ArrayBuffer(64), info: MSG_KEY_INFO },
      mkMaterial,
      256
    ) as ArrayBuffer
  );
