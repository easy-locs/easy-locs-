    ...
    <DashboardLayout>
      ...
      <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-4">
          {[
            { icon: TrendingUp, label: t("page.finances.collected_month"), value: fmt(kpis.revenueThisMonth), sub: `${t("page.finances.on")} ${fmt(kpis.expectedThisMonth)}`, path: "/dashboard/rental-management?tab=payments", iconClassName: "text-success", loading: dataLoading },
            { icon: TrendingDown, label: t("page.finances.unpaid"), value: fmt(kpis.totalUnpaid), sub: `${filteredRentCalls.filter(r => !r.paid).length} ${t("page.finances.call_count")}`, path: "/dashboard/dunning", iconClassName: "text-destructive", loading: dataLoading },
            { icon: PiggyBank, label: t("page.finances.total_collected"), value: fmt(kpis.totalRevenue), sub: `${t("page.finances.on")} ${fmt(kpis.totalExpected)}`, path: "/dashboard/rental-management?tab=payments", iconClassName: "text-accent", loading: dataLoading },
            { icon: Wallet, label: t("page.finances.total_expenses"), value: fmt(kpis.totalExpenses), sub: `${filteredExpenses.length} ${t("page.finances.expense_count")}`, path: "/dashboard/expenses", iconClassName: "text-destructive", loading: dataLoading },
            { icon: BarChart3, label: t("page.finances.net_result"), value: fmt(kpis.netResult), sub: `${t("page.finances.total_collected")} - ${t("page.finances.total_expenses")}`, iconClassName: "text-accent", valueClassName: kpis.netResult >= 0 ? "text-success" : "text-destructive", loading: dataLoading },
            { icon: CheckCircle, label: t("page.finances.collection_rate"), value: `${kpis.occupancyRate}%`, path: "/dashboard/rental-management?tab=payments", iconClassName: "text-accent", loading: dataLoading },
          ].map(kpi => (
