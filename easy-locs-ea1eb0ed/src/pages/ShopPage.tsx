...
function hasCurrency(entry: FallbackEntry): entry is FallbackServiceProvider | FallbackHotel {
  return "currency" in entry;
}

function findFallbackEntity(slug: string) {
  const allFallbacks: FallbackEntry[] = [
    ...FALLBACK_SHOPS,
    ...FALLBACK_GROCERY,
    ...FALLBACK_SERVICES,
    ...FALLBACK_HOTELS,
  ];
  const match = allFallbacks.find(e => e.slug === slug);
  if (!match) return null;
  return {
    ...match,
    currency: hasCurrency(match) ? match.currency : "AED",
    _fallback: true,
  };
}
...
export default function ShopPage() {
    ...
    queryFn: async () => {
      try {
        ...
        if (canonical.sourceTable === "storefront_pages") {
          const sfData = await storefrontService.fetchPageById(canonical.canonicalId);
        ...
        console.warn("[ShopPage] canonical resolution failed, trying fallback", err);
      }

      const fallbackEntity = findFallbackEntity(shopSlug!);
  ...
  const isFallbackEntity = !!(shop as Record<string, unknown>)?._fallback;

  // ── Catalog ──
  const { data: catalogItems = [] } = useQuery({
    queryKey: ["storefront-catalog", shop?.id],
    queryFn: async () => {
      if (!shop || isFallbackEntity) return [];
      return storefrontService.fetchCatalogItems(shop.id);
    },
    enabled: !!shop?.id && !isFallbackEntity,
  });
  ...
  const { data: categories = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!shop || isFallbackEntity) return [];
      return storefrontService.fetchCatalogCategories(shop.id);
    },
    enabled: !!shop?.id && !isFallbackEntity,
  });
  ...
  const isFoodVertical = ["food", "grocery"].includes((shop.vertical || "").toLowerCase());
  const contextBanners = isFoodVertical
    ? getTopBanners({ country: shop.country, city: shop.city }, 1)
    : [];

  const catalogLabel = (() => {
    const v = (shop.vertical || "").toLowerCase();
    if (v === "property" || v === "real_estate" || v === "stays") return "Rooms";
    if (v === "services") return "Services";
    if (v === "food" || v === "grocery") return "Menu";
    return "Products";
  })();

  const TABS: { id: ShopTab; label: string }[] = [
    { id: "overview", label: "Overview" },
    { id: "menu", label: `${catalogLabel}${catalogItems.length > 0 ? ` (${catalogItems.length})` : ""}` },
    ...
    <>
        ...
        jsonLd={[
          {
            ...
            ...(shop.rating != null ? {
              aggregateRating: {
                ...
                reviewCount: shop.reviews_count ?? 1,
              },
            } : {}),
            ...(catalogItems.length > 0 ? {
              makesOffer: catalogItems.slice(0, 10).map((item: any) => ({
                "@type": "Offer",
                itemOffered: { "@type": "Product", name: item.name },
                ...(item.price > 0 ? { price: item.price, priceCurrency: shop.currency || shop.default_currency || "AED" } : {}),
              })),
            } : {}),
          },
          {
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: verticalUI.displayTitle, item: buildAppUrl(`/browse/${shop.vertical || "shops"}`) },
      ...
      <div className="app-mobile-page bg-background pb-24">
        <nav aria-label="Breadcrumb" className="px-4 py-2 flex items-center gap-1 text-xs text-muted-foreground overflow-x-auto">
          ...
          <button onClick={() => navigate(`/browse/${shop.vertical || "shops"}`)} className="hover:text-foreground shrink-0">{verticalUI.displayTitle}</button>
          {shop.city && (
            <>
              <ChevronRight className="w-3 h-3 shrink-0" />
              <span className="shrink-0">{shop.city}</span>
            </>
          )}
          <ChevronRight className="w-3 h-3 shrink-0" />
