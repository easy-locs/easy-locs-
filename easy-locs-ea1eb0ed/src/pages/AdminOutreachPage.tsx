...
export default function AdminOutreachPage() {
  ...
  const loadData = async () => {
    ...
    const count = await adminOpsService.countWhere("merchant_onboarding_profiles", "onboarding_status", "imported_not_claimed");
  ...
  const handleBulkSend = async () => {
    setSending(true);
    try {
      const unclaimed = await adminOpsService.fetchUnclaimedMerchants(50);
