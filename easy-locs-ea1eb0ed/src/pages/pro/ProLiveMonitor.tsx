    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'hsl(220 20% 55%)', fontSize: 14, margin: '4px 0 0' }}>Real-time engine activity and health status</p>
        </div>
        <button
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 24 }}>
        <div style={{ background: CARD_BG, borderRadius: 10, padding: 16, border: `1px solid ${NAVY_LIGHT}` }}>
          ...
          <div style={{ color: 'hsl(220 20% 55%)', fontSize: 12 }}>Total Engines</div>
        </div>
        <div style={{ background: CARD_BG, borderRadius: 10, padding: 16, border: `1px solid ${NAVY_LIGHT}` }}>
          <div style={{ color: '#22c55e', fontSize: 20, fontWeight: 700 }}>{alive}</div>
          <div style={{ color: 'hsl(220 20% 55%)', fontSize: 12 }}>Active</div>
        </div>
        <div style={{ background: CARD_BG, borderRadius: 10, padding: 16, border: `1px solid ${NAVY_LIGHT}` }}>
          <div style={{ color: '#f59e0b', fontSize: 20, fontWeight: 700 }}>{slow}</div>
          <div style={{ color: 'hsl(220 20% 55%)', fontSize: 12 }}>Warnings</div>
        </div>
        <div style={{ background: CARD_BG, borderRadius: 10, padding: 16, border: `1px solid ${NAVY_LIGHT}` }}>
          <div style={{ color: '#ef4444', fontSize: 20, fontWeight: 700 }}>{dead}</div>
          <div style={{ color: 'hsl(220 20% 55%)', fontSize: 12 }}>Dead</div>
        </div>
        <div style={{ background: CARD_BG, borderRadius: 10, padding: 16, border: `1px solid ${NAVY_LIGHT}` }}>
          <div style={{ color: GOLD, fontSize: 20, fontWeight: 700 }}>0.2%</div>
          <div style={{ color: 'hsl(220 20% 55%)', fontSize: 12 }}>Error Rate</div>
        </div>
      </div>
              ...
              return (
                <div key={engine.name} style={{ background: CARD_BG, borderRadius: 10, padding: '12px 14px', border: `1px solid ${NAVY_LIGHT}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: 'hsl(220 20% 50%)', fontSize: 10 }}>{engine.execPerMin}/min — {engine.avgMs}ms avg</div>
                  </div>
                  <span style={{ color: 'hsl(220 20% 45%)', fontSize: 10, flexShrink: 0 }}>{engine.lastPing}</span>
                </div>
              );
            })}
          </div>
            ...
            {ACTIVITY.map((entry, i) => (
              <div key={i} style={{ padding: '10px 14px', borderBottom: `1px solid ${NAVY_LIGHT}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ color: 'hsl(220 20% 50%)', fontSize: 11 }}> {entry.action} </span>
                  <span style={{ color: 'hsl(220 20% 65%)', fontSize: 11 }}>{entry.entity}</span>
                </div>
                <span style={{ color: 'hsl(220 20% 45%)', fontSize: 10, flexShrink: 0 }}>{entry.duration}</span>
                <span style={{ color: 'hsl(220 20% 40%)', fontSize: 10, flexShrink: 0 }}>{entry.time}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: CARD_BG, borderRadius: 12, padding: 20, border: `1px solid ${NAVY_LIGHT}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ color: 'hsl(220 20% 55%)', fontSize: 11 }}>Active Jobs</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: '#f59e0b', fontSize: 20, fontWeight: 700 }}>0</div>
              <div style={{ color: 'hsl(220 20% 55%)', fontSize: 11 }}>Delayed</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: 'hsl(220 20% 45%)', fontSize: 20, fontWeight: 700 }}>0</div>
              <div style={{ color: 'hsl(220 20% 55%)', fontSize: 11 }}>Collisions</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: CARD_BG, borderRadius: 12, padding: 20, border: `1px solid ${NAVY_LIGHT}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#ef444410', borderRadius: 8, border: '1px solid #ef444430' }}>
            ...
            <div>
              ...
              <div style={{ color: 'hsl(220 20% 55%)', fontSize: 11 }}>890ms avg (threshold: 500ms) — 28s ago</div>
            </div>
          </div>
        </div>
      </div>
