...
export default function AdminWalletDiagnosticsPage() {
  ...
  const lookup = async () => {
    ...
    setLoading(true);
    try {
      const [orderData, splitsData, txData, auditData] = await Promise.all([
        adminOpsService.fetchOrderForDiagnostics(orderId),
        adminOpsService.fetchWalletOrderSplits(orderId),
        adminOpsService.fetchWalletTransactionsByContext(orderId),
        adminOpsService.fetchWalletAuditLogs(),
      ]);
      setOrder(orderData as OrderDiag);
      setSplits(splitsData);
      setTransactions(txData);

      const txIds = txData.map((t: any) => t.id);
      const ledgerData = await adminOpsService.fetchLedgerByTransactionIds(txIds);
