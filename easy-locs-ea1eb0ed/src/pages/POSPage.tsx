...
export default function POSPage() {
  ...
  const processPayment = async () => {
    ...
    setProcessing(true);
    try {
      const orgMember = await posService.fetchOrgMembership(user.id);
      const orgId = orgMember?.org_id;

      const shop = await posService.fetchSellerShop(user.id);

      const order = await posService.createStorefrontOrder({
      ...
      await posService.insertStorefrontOrderItems(orderItems);
      ...
      await posService.updateStorefrontOrderStatus(order.id, { status: "accepted", payment_status: "paid", wallet_reference_code: null });
