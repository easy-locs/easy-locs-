                            ...
                            <div className="grid grid-cols-1 gap-2 text-sm">
                              {ev.guestEmail && (
                                <button onClick={async () => {
                                  ...
                                  const path = await navigateToOrbitThread({ targetEmail: ev.guestEmail, targetName: ev.guestName || "Guest" });
                    ...
                    {ev.source !== "blocked" && (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          {ev.guestEmail && (
                            <Button variant="outline" size="sm" className="w-full gap-1.5 text-xs h-9" onClick={async () => {
                              const { navigateToOrbitThread } = await import("@/lib/orbit/navigate-to-thread");
                              const path = await navigateToOrbitThread({ targetEmail: ev.guestEmail, targetName: ev.guestName || "Guest" });
                              if (path) routeNavigate(path);
                            }}>
