...
export default function ProductPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      return storefrontService.fetchCatalogItemById(productId!);
    },
    enabled: !!productId,
  });
            ...
            {shop && (
              ...
              >
                <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-primary/10 shrink-0">
                  {shop.logo_url
                    ? <img loading="lazy" src={shop.logo_url} alt={`${shop.name || "Shop"} logo`} className="w-9 h-9 rounded-xl object-cover" />
