...
export default function MerchantPosPage() {
  ...
  useEffect(() => {
    ...
    posService.fetchMenuItems(merchantProfileId).then(setItems).catch(() => {});
  }, [merchantProfileId]);
  ...
  const loadRecentOrders = async () => {
    if (!merchantProfileId) return;
    const data = await posService.fetchRecentOrders(merchantProfileId);
  ...
  const createOrderDraft = async () => {
    ...
    try {
      ...
      await posService.insertOrderItems(orderItems);
