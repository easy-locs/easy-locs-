...
export default function TravelHub() {
  ...
  const hasStayParams = !!(destination || propertyType);
  ...
  return (
    <div className="app-mobile-page bg-background pb-24">
      ...
      <div className="px-4 mt-5">
        ...
        {activeTab === "stays" && <StayQuickSearch onSearch={() => navigate("/travel/stays")} initialDestination={destination} />}
...
function StayQuickSearch({ onSearch, initialDestination = "" }: { onSearch: () => void; initialDestination?: string }) {
  return (
    <div className="space-y-2 p-4 rounded-2xl bg-card border border-border/20 shadow-sm">
      <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
      ...
      <InputIcon icon={<MapPin />} placeholder="City, area or property name" defaultValue={initialDestination ? initialDestination.charAt(0).toUpperCase() + initialDestination.slice(1) : undefined} />
      <div className="grid grid-cols-2 gap-2">
        <InputIcon icon={<Calendar />} type="date" />
        <InputIcon icon={<Calendar />} type="date" />
      </div>
...
function InputIcon({ icon, placeholder, type, className, defaultValue }: { icon: React.ReactNode; placeholder?: string; type?: string; className?: string; defaultValue?: string }) {
  return (
    <div className={`relative ${className || ""}`}>
      <span className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-muted-foreground [&>svg]:h-3.5 [&>svg]:w-3.5">{icon}</span>
      <Input type={type} placeholder={placeholder} defaultValue={defaultValue} className="pl-8 h-9 text-xs rounded-xl" />
    </div>
  );
}
