...
function StayStoriesSection() {
  const { data: stories = [] } = useStoryFeed("stay_trending");
  if (!stories.length) return null;
  return (
    <div className="-mx-4 mb-3">
      <StoryPreviewRail title="Trending stays" stories={stories.slice(0, 8)} size="small" feedKey="stay_trending" surface="stay_hub" />
    </div>
  );
}
...
export default function TravelStayHub() {
  ...
  const ui = useMemo(() => resolveCanonicalUI("stay", activeTab), [activeTab]);

  const { data: listings = [], isLoading } = useVerticalListings("stay", activeTab);
      ...
      <div className="relative overflow-hidden" style={{ height: 380 }}>
        ...
        <div className="relative z-10 px-4 pt-12 pb-4">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <button
              onClick={() => window.history.length > 1 ? navigate(-1) : navigate("/travel")}
      ...
      <div className="px-4 mt-6">
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="space-y-3">
            {filtered.length > 0 && (
              <PremiumMerchantCard
                to={`/travel/hotel/${filtered[0].id}`}
                image={filtered[0].banner_url || filtered[0].logo_url}
                name={filtered[0].name}
                category={filtered[0].address || getSubcategoryLabel("stay", filtered[0].subcategory || "")}
            ...
            {filtered.slice(1).map((item, i) => (
              <PremiumMerchantCard
                key={item.id}
                to={`/travel/hotel/${item.id}`}
                image={item.banner_url || item.logo_url}
                name={item.name}
                category={item.address || getSubcategoryLabel("stay", item.subcategory || "")}
        ...
        <div className="mt-8 mb-6">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {[
              { name: "Dubai Marina", emoji: "🌃", img: heroCover("stay") },
              { name: "Palm Jumeirah", emoji: "🌴", img: heroCover("stay") },
              { name: "Abu Dhabi", emoji: "🕌", img: heroCover("stay") },
              { name: "Ras Al Khaimah", emoji: "🏔️", img: heroCover("stay") },
              { name: "Sharjah", emoji: "🏛️", img: heroCover("stay") },
              { name: "Fujairah", emoji: "🏖️", img: heroCover("stay") },
            ].map((dest, i) => (
              <motion.button
                key={dest.name}
