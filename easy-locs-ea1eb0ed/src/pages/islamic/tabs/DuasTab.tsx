...
import { ChevronDown, RotateCcw, Search, Heart, Copy, Share2 } from "lucide-react";
...
function loadFavorites(): string[] {
  try {
    const raw = localStorage.getItem(LS_DUA_FAVORITES_KEY);
    if (raw) return JSON.parse(raw);
  } catch {}
  return [];
}
...
function loadDailyCount(): { date: string; count: number } {
  try {
    const raw = localStorage.getItem(LS_DAILY_COUNT_KEY);
    if (raw) return JSON.parse(raw);
  } catch {}
  return { date: "", count: 0 };
}
...
export default function DuasTab() {
  ...
  const copyDua = useCallback(async (dua: Dua) => {
    const text = `${dua.arabic}\n\n${dua.transliteration}\n\n${dua.french}${dua.source ? `\n\nSource : ${dua.source}` : ""}`;
    try {
      await navigator.clipboard.writeText(text);
      toast.success("Dua copié");
    } catch {
      ...
      {showFavorites && (
        <div className="space-y-2">
          <h3 className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
            Favoris ({favoriteDuas.length})
          </h3>
          {favoriteDuas.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">Aucun favori</p>
          )}
...
function DuaCard({
  dua, count, isFavorite, onIncrement, onReset, onToggleFavorite, onCopy, onShare,
}: {
  dua: Dua; count: number; isFavorite: boolean;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-right text-base leading-loose flex-1" style={{ fontFamily: "'Amiri', 'Traditional Arabic', serif", direction: "rtl" }}>
          {dua.arabic}
        </p>
        <div className="flex gap-1 shrink-0">
          ...
          <button onClick={onCopy} className="w-7 h-7 rounded-full flex items-center justify-center">
            <Copy size={12} className="text-muted-foreground" />
          </button>
          <button onClick={onShare} className="w-7 h-7 rounded-full flex items-center justify-center">
