...
export default function AdminSourceAuditPage() {
  ...
  async function loadStats() {
    setLoading(true);
    try {
      const data = await adminOpsService.fetchStorefrontSources();
        ...
        {stats.map(s => (
          <div key={s.source_type} className="rounded-2xl bg-card border border-border/20 p-3">
            ...
            <div className="flex gap-3 mt-2">
              <div className={`text-[0.625rem] px-2 py-0.5 rounded-full font-bold ${
              ...
              <div className="text-[0.625rem] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-bold">
                {s.claimed} claimed
              </div>
              <div className="text-[0.625rem] px-2 py-0.5 rounded-full bg-muted text-muted-foreground font-bold">
        ...
        <div className="flex gap-2 flex-wrap">
          {VERTICALS.map(v => (
            <button
              ...
              className={`text-[0.6875rem] px-3 py-1.5 rounded-full font-bold transition-colors ${
                selectedVertical === v
          ...
          {trustedSources.map(s => (
            <div key={s.key} className="flex items-center justify-between rounded-xl bg-card border border-border/20 px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                <div className="text-[0.625rem] text-muted-foreground">parser: {s.parser}</div>
                <div className={`text-[0.625rem] px-2 py-0.5 rounded-full font-bold ${
