...
export default function AdminRankingControlPage() {
  ...
  const { data: stats, refetch } = useQuery({
    ...
    queryFn: async () => {
      const data = await adminOpsService.fetchRankingStats() as any[];
      if (!data.length) return { total: 0, byClass: {}, avgScore: 0, claimReady: 0, boostReady: 0 };
  ...
  const { data: entities } = useQuery({
    ...
    queryFn: () => adminOpsService.fetchRankingEntities(50) as Promise<any[]>,
  });
