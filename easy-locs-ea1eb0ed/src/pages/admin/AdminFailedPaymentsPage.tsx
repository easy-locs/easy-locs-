  ...
  return (
    <div className="min-h-[100dvh] bg-background pb-24">
      <div className="flex items-center gap-3 px-4 pt-6 pb-4">
        ...
        <div>
          <h1 className="text-lg font-bold text-foreground">Failed Payments</h1>
          <p className="text-xs text-muted-foreground">Orders with failed payment status</p>
        </div>
      </div>
        ...
        <div className="px-4 space-y-3">
          {rows.map((row: any) => (
            <div key={row.id} className="rounded-2xl border border-border/20 bg-card p-4">
              ...
              <span className="inline-block mt-2 rounded-full bg-destructive/10 text-destructive px-2.5 py-0.5 text-[10px] font-bold">
                Failed
              </span>
            </div>
          ))}
