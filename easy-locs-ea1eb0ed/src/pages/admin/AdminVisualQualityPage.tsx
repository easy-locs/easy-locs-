...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${cls}`}>{severity}</span>;
}
            ...
            <div key={p.page} className="space-y-1.5">
              ...
              {p.issues.map((issue, i) => (
                <div key={i} className="flex items-start gap-2 text-[11px]">
                  ...
                  {issue.fixed && <span className="text-emerald-500 text-[10px]">✓ fixed</span>}
                </div>
              ))}
            </div>
          ))}
