  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [total, live, searchOnly, comingSoon] = await Promise.all([
        db("seed_merchants").select("id", { count: "exact", head: true }).neq("visibility_mode", "hidden"),
      ...
      const [noName, noCategory, noVertical, brokenRoute] = await Promise.all([
        db("seed_merchants").select("id", { count: "exact", head: true }).neq("visibility_mode", "hidden").or("name.is.null,name.eq."),
        db("seed_merchants").select("id", { count: "exact", head: true }).neq("visibility_mode", "hidden").or("category.is.null,category.eq.unknown,category.eq."),
        db("seed_merchants").select("id", { count: "exact", head: true }).neq("visibility_mode", "hidden").or("vertical.is.null,vertical.eq.unknown"),
        db("seed_merchants").select("id", { count: "exact", head: true }).neq("visibility_mode", "hidden").eq("route_status", "broken"),
      ]);
            ...
            <div className="p-3 space-y-1">
              {[
                ...
                <div key={`${m.from}-${m.to}`} className="flex items-center gap-2 rounded-lg bg-muted/30 px-3 py-1.5">
                  ...
                  <span className={`text-[10px] font-bold ${m.status === "connected" ? "text-emerald-500" : "text-amber-500"}`}>
                  ...
                  return (
                    <div key={a.id} className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold ${sevColor}`}>{a.severity?.toUpperCase()}</span>
                        <span className="text-[10px] font-bold text-foreground">{a.action_type}</span>
                        {a.auto_applied && <span className="text-[10px] bg-emerald-500/20 text-emerald-400 rounded px-1">AUTO</span>}
                        <span className="text-[10px] text-muted-foreground ml-auto">{a.engine_source}</span>
                      </div>
