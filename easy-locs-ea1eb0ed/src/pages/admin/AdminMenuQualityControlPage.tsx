  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [allRes, cleanRes, rebuiltRes, blockedRes, garbageRes] = await Promise.all([
        ...
        db("seed_merchants").select("id", { count: "exact", head: true }).eq("vertical", "food").eq("menu_quality_flag", "clean"),
        db("seed_merchants").select("id", { count: "exact", head: true }).eq("vertical", "food").eq("menu_quality_flag", "rebuilt"),
        db("seed_merchants").select("id", { count: "exact", head: true }).eq("vertical", "food").eq("visibility_mode", "hidden"),
        db("seed_merchants").select("id", { count: "exact", head: true }).eq("vertical", "food").eq("menu_quality_flag", "garbage"),
      ]);
          ...
          <div className="space-y-2">
            ...
            {data.worst.map((m: any) => (
              <div key={m.id} className="rounded-2xl border border-border/20 bg-card p-3">
                ...
                {m.blocking_reason && (
                  <p className="text-[10px] text-red-400/80 mt-1 truncate">{m.blocking_reason}</p>
                )}
              </div>
            ))}
          </div>
