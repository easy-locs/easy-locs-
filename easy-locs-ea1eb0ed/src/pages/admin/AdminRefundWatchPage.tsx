...
export default function AdminRefundWatchPage() {
  ...
  const disputed = rows.filter((r: any) => r.status === "disputed").length;
  const refunded = rows.filter((r: any) => r.status === "refunded").length;
  ...
  return (
    <div className="min-h-[100dvh] bg-background pb-24">
      <div className="flex items-center gap-3 px-4 pt-6 pb-4">
        ...
        <div>
          <h1 className="text-lg font-bold text-foreground">Refund Watch</h1>
          <p className="text-xs text-muted-foreground">Refund and dispute queue</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 pb-4">
        ...
        <Metric title="Cancelled" value={String(cancelled)} />
      </div>
            ...
            <div key={row.id} className="rounded-2xl border border-border/20 bg-card p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{Number(row.total_amount ?? 0).toFixed(2)} {row.currency ?? "AED"}</p>
                  <p className="text-[11px] text-muted-foreground/70">{row.updated_at ? new Date(row.updated_at).toLocaleString() : ""}</p>
                </div>
