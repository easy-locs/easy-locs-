...
export default function AdminHomeEnginePage() {
  ...
  const rerun = async () => {
    try {
      const merchants = await adminOpsService.fetchSeedMerchantsForScoring(150) as any[];
      let ok = 0;
      for (const m of merchants) {
        ...
        await adminOpsService.updateMerchantVisibilityScore(m.id, Number(score.toFixed(2)));
        ok++;
      }
