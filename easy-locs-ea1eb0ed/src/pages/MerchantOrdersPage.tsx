 ...
 * Real-time via repository subscription.
 */
...
const TABS: { key: OrderStatus; label: string; icon: any; color: string }[] = [
  ...
  { key: "ready", label: "Ready", icon: PackageCheck, color: "hsl(var(--primary))" },
...
export default function MerchantOrdersPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      const data = await fetchAllConciergeOrders();
      if (data) {
    ...
    const unsub = subscribeMerchantOrders((newOrder: any) => {
  ...
  const handleStatusChange = useCallback(async (orderId: string, newStatus: OrderStatus) => {
    ...
    await updateConciergeOrderStatus(orderId, dbStatus);
          ...
          return (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              ...
              style={{
                ...
                border: `1px solid ${active ? "hsl(var(--primary) / 0.2)" : "hsl(var(--border) / 0.06)"}`,
              }}>
      ...
      <div className="flex-1 px-4 pb-6 space-y-3">
        {filteredOrders.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm font-medium" style={{ color: "hsl(var(--muted-foreground) / 0.5)" }}>No {activeTab} orders</p>
          </div>
        )}
