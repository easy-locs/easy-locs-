    ...
    <>
        ...
        jsonLd={[
          ...
          },
          {
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 2, name: "Marketplace", item: buildAppUrl("/marketplace") },
              ...(profile.city ? [{ "@type": "ListItem", position: 3, name: profile.city }] : []),
              { "@type": "ListItem", position: profile.city ? 4 : 3, name },
            ],
          },
        ]}
      />
      <SubPageShell noContentPad className="pb-16 sm:pb-0">
        <nav aria-label="Breadcrumb" className="px-4 py-2 flex items-center gap-1 text-xs text-muted-foreground overflow-x-auto border-b border-border/50">
          ...
          <Link to="/marketplace" className="hover:text-foreground shrink-0">Marketplace</Link>
          {profile.city && (
            <>
              <ChevronRight className="w-3 h-3 shrink-0" />
              <span className="shrink-0">{profile.city}</span>
            </>
          )}
          <ChevronRight className="w-3 h-3 shrink-0" />
