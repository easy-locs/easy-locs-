...
function mapRatePlan(p: any): HotelRatePlan {
  return {
    id: p.id,
    name: p.name,
...
function mapAvailDay(a: any): HotelAvailDay {
  return {
    date: a.night_date,
...
function mapRoom(room: any, plans: any[], avail: any[]): HotelRoom {
  const availDays = avail.map(mapAvailDay);
  ...
  return {
    ...
    rate_plans: plans.map(mapRatePlan),
...
function mapHotel(hotel: any, rooms: HotelRoom[]): HotelData {
  return {
    id: hotel.id,
    name: hotel.name,
    slug: hotel.slug,
...
async function enrichRoom(roomId: string, room: any, from: string, to: string): Promise<HotelRoom> {
  const [plansResult, availResult] = await Promise.all([
    (db as any).from("hotel_rate_plans").select("*").eq("room_id", roomId).eq("active", true),
    (db as any).from("hotel_inventory_calendar").select("*").eq("room_type_id", roomId).gte("night_date", from).lte("night_date", to).order("night_date"),
  ]);

  return mapRoom(room, plansResult.data ?? [], availResult.data ?? []);
}
...
export function useHotelDetail(hotelId: string | undefined) {
  return useQuery({
    ...
    queryFn: async (): Promise<HotelData | null> => {
      ...
      const { data: hotel, error } = await (db as any)
        .from("hotels")
        .select("*")
        .eq("id", hotelId)
        .single();
      ...
      const enrichedRooms = await Promise.all(
        (rooms ?? []).map((room: any) => enrichRoom(room.id, room, from, to))
      );

      return mapHotel(hotel, enrichedRooms);
    },
    enabled: !!hotelId,
    staleTime: 60_000,
  });
