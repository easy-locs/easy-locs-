...
export function useRentalRentCalls() {
  ...
  const handlePayRent = useCallback(async (payment: any, tenantName: string) => {
    ...
    try {
      const data = await rentalRepo.invokeRentPayment({
        rentCallId: payment.id, amount: payment.total_amount, tenantName, month: payment.month, orgId,
      });
  ...
  const handleNotifyRentCall = useCallback(async (payment: any, tenant: { id: string; email: string; tenant_user_id?: string | null; name: string }) => {
    ...
    try {
      ...
      const emailData = await rentalRepo.invokeSendEmail({
        ...
        html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#ffffff;">
          ...
          <div style="text-align:center;margin:24px 0;">
            <a href="${appUrl}/wallet" style="display:inline-block;background:#d4a853;color:#1a1a1a;font-weight:600;text-decoration:none;padding:12px 32px;border-radius:8px;font-size:15px;">${escapeEmailHtml(t("page.rental.pay_rent_btn"))}</a>
          </div>
        </div>`,
      });
      if (emailData && emailData.success === false) throw new Error(emailData?.error || "error");
      ...
      platformBus.emit(APP_EVENTS.RENTAL_RENT_CALL_CREATED, { rentCallId: payment.id }, "rental");
  ...
  const autoGenerateFirstRentCall = useCallback(async (tenantId: string, form: any, properties: any[]) => {
    ...
    try {
      ...
      if (inserted?.id) {
        ...
        platformBus.emit(APP_EVENTS.RENTAL_RENT_CALL_CREATED, { rentCallId: inserted.id }, "rental");
      }
