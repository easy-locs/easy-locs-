...
export function useBookingLifecycle(opts: UseBookingLifecycleOpts = {}) {
  ...
  const sendPaymentLink = async (booking: any) => {
    ...
    const threadPath = await navigateToOrbitThread({ targetEmail: booking.booker_email, targetName: booking.booker_name || "Client" });
    if (threadPath) window.location.href = threadPath;
