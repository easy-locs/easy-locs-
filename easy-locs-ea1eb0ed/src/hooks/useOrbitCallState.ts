...
export function useOrbitCallState() {
  ...
  const startOutgoing = useCallback((payload: {
    sessionId: string;
  ...
  const patchCall = useCallback((patch: Partial<OrbitActiveCall>) => {
    setActiveCall((prev) => (prev ? { ...prev, ...patch } : prev));
  }, []);

  const setUiState = useCallback((uiState: OrbitCallUiState) => {
    setActiveCall((prev) => (prev ? { ...prev, uiState } : prev));
  }, []);

  const endCall = useCallback((uiState: OrbitCallUiState = "ended") => {
    setActiveCall((prev) =>
      prev
