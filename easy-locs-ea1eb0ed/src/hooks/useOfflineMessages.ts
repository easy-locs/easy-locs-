...
export function useOfflineMessages({ userId, orgId, threadId }: UseOfflineMessagesOptions) {
  ...
  stableRef.current.isOnline = isOnline;
  stableRef.current.queueCount = queueCount;
  stableRef.current.isSyncing = isSyncing;
  stableRef.current.queueMessage = queueMessage;
  stableRef.current.flushQueue = flushQueue;
  stableRef.current.cacheMessages = cacheMessages;
  stableRef.current.getCachedMessages = getCachedMessages;
  stableRef.current.getThreadPending = getThreadPending;
