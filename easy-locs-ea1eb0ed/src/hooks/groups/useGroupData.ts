...
export function useGroupData() {
  ...
  const addMemberToGroup = useCallback(async (groupId: string, email: string, role: MemberRole) => {
    if (!email.trim() || !groupId) return false;
  ...
  const addMember = useCallback(async (email: string, role: MemberRole) => {
    ...
    return addMemberToGroup(activeGroup.id, email, role);
  }, [activeGroup, addMemberToGroup]);
  ...
  const leaveGroup = useCallback(async (groupId?: string) => {
    ...
    await groupsRepo.deleteGroupMemberByUser(gid, user.id);
    toast.success("Left group");
    if (activeGroup?.id === gid) { setActiveGroup(null); setMessages([]); setMembers([]); }
    await loadGroups();
  ...
  const deleteGroup = useCallback(async (groupId?: string) => {
    const gid = groupId || activeGroup?.id;
    if (!gid) return;
    await groupsRepo.deleteGroupMembers(gid);
    await groupsRepo.deleteConversation(gid);
  ...
  return {
    ...
    addMember, addMemberToGroup, removeMember, leaveGroup, deleteGroup,
