...
type SupabaseStorageClient = {
  from(bucket: string): {
    upload(path: string, data: File | Blob, opts?: Record<string, unknown>): Promise<{ error: Error | null }>;
...
function getStorage(): SupabaseStorageClient {
  return (supabase as unknown as { storage: SupabaseStorageClient }).storage;
}
...
async function resolveUrl(path: string): Promise<string> {
  const { data: signedData } = await getStorage()
    .from(BUCKET)
    .createSignedUrl(path, 60 * 60 * 24 * 365);

  if (signedData?.signedUrl) return signedData.signedUrl;

  const { data: publicData } = getStorage()
    .from(BUCKET)
    .getPublicUrl(path);
...
// ── Resilient upload for large files (≥ 6 MB) ──
// Supabase storage does not support TUS-style resumable or server-side
// chunk assembly. Strategy: single upload with enhanced retry resilience,
// exponential backoff, network-aware waiting, and smooth progress tracking.
// No redundant double-upload — exactly one PUT for the full file.
async function uploadChunked(
  ...
  await withRetry(async () => {
    const { error } = await getStorage()
      ...
      .upload(path, data, { cacheControl: "3600", upsert: false });
    if (error) throw error;
  }, MAX_RETRIES + 1, cb.signal);

  for (let i = 1; i <= totalChunks; i++) {
    progress.set(Math.min(Math.round((i / totalChunks) * 85), 85));
  }
  ...
  if (opts.compress && file.type.startsWith("image/")) {
    ...
    try {
      ...
      if (result.ratio < 0.95) {
        ...
        compressed = true;
        if (opts.targetFormat === "image/webp" && result.blob.type === "image/webp") {
          ext = "webp";
        }
      }
    } catch {
