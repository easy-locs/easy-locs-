...
export function useConversationViewModel(conversationId: string | null): ConversationViewModel | null {
  const conversation = useOrbitMessagingStore((s) => conversationId ? s.conversations[conversationId] : undefined);
  const messageIds = useOrbitMessagingStore((s) => conversationId ? s.messagesByConversation[conversationId] : undefined);
  // SCOPED read: only messages referenced by this conversation's bucket — NOT the full map
  const scopedMessages = useOrbitMessagingStore((s) => {
